...
description: Mine this machine's own agent history and memory for this repository (Claude Code session transcripts and project memory, Codex session rollouts and memories), find corrections and lessons the shared guides are missing, and promote them into the canonical AGENTS.md, skills, or docs file while deduping and trimming. Documentation-only; leaves edits uncommitted. Use when asked to refresh, audit, or trim agent guidance, after a run of sessions with corrections, or on the monthly cadence.
---
