...
class GemstoneStakeStore(private val stakeDao: StakeDao, private val assetsDao: AssetsDao) : GemStakeStore {
    ...
    fun observeValidators(assetId: AssetId, providerType: StakeProviderType): Flow<List<DelegationValidator>> = stakeDao.getValidators(assetId, providerType).map { it.toDTO() }

    suspend fun getValidator(assetId: AssetId, validatorId: String): DelegationValidator? = stakeDao.getValidator(assetId, validatorId)?.toDTO()

    fun observeDelegations(walletId: WalletId, assetId: AssetId, providerType: StakeProviderType): Flow<List<Delegation>> = stakeDao.getDelegations(walletId, assetId, providerType).map { rows -> rows.mapNotNull { it.toModel() } }

    fun observeDelegation(walletId: WalletId, validatorId: String, delegationId: String): Flow<Delegation?> = stakeDao.getDelegation(walletId, validatorId, delegationId).map { it?.toModel() }
}
