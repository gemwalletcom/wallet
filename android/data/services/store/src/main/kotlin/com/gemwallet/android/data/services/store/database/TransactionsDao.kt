...
import com.gemwallet.android.data.services.store.database.entities.DbTransactionAsset
import com.gemwallet.android.data.services.store.database.entities.DbTransactionExtended
import com.gemwallet.android.data.services.store.database.entities.DbTransactionListItem
...
interface TransactionsDao {
    ...
    fun updateTransactionHash(oldId: TransactionId, walletId: WalletId, hash: String, updatedAt: Long = System.currentTimeMillis()) {
        ...
        if (target == null) copyTransactionAssets(oldId.identifier, newId.identifier)
    }
    ...
    fun replaceTransactionAssets(assets: Map<String, List<String>>) {
        assets.forEach { (transactionId, assetIds) ->
            deleteTransactionAssets(transactionId)
            addTransactionAssets(assetIds.map { DbTransactionAsset(transactionId, it) })
        }
    }

    @Query("DELETE FROM transactions_assets WHERE tx_id = :transactionId")
    fun deleteTransactionAssets(transactionId: String)

    @Insert(onConflict = OnConflictStrategy.IGNORE)
    fun addTransactionAssets(assets: List<DbTransactionAsset>)

    @Query("INSERT OR IGNORE INTO transactions_assets (tx_id, asset_id) SELECT :newId, asset_id FROM transactions_assets WHERE tx_id = :oldId")
    fun copyTransactionAssets(oldId: String, newId: String)

    @Query("DELETE FROM transactions_assets WHERE tx_id = :transactionId AND NOT EXISTS (SELECT 1 FROM transactions WHERE transactions.id = :transactionId)")
    fun deleteUnreferencedTransactionAssets(transactionId: String)
