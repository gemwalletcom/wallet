...
import com.gemwallet.android.data.services.store.database.entities.toInAppNotification
...
class InAppNotificationsQuery @Inject constructor(private val notificationsDao: InAppNotificationsDao) {
    ...
    operator fun invoke(walletId: String): Flow<List<InAppNotification>> = notificationsDao.getNotifications(walletId).map { records -> records.map { it.toInAppNotification() } }
}
