...
class RecentActivityQuery @Inject constructor(private val assetsDao: AssetsDao) {
    ...
    operator fun invoke(walletId: WalletId, types: List<RecentActivityType> = RecentActivityType.entries, filters: Set<AssetsQueryFilter> = emptySet(), limit: Int = 20): Flow<List<RecentAsset>> =
        assetsDao.getRecentAssets(walletId.id, types, filters, limit)
            .map { rows -> rows.mapNotNull { row -> row.asset.toDTO()?.let { RecentAsset(asset = it, createdAt = row.addedAt) } } }
}
