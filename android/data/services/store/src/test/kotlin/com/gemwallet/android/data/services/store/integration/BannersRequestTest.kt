...
class BannersRequestTest {
    ...
    fun nativeAssetListsTheWalletAndSharedBannersWithTheStoredAsset() = runBlocking(Dispatchers.IO) {
        val banners = BannersRequest(database.bannersDao())("wallet-1", asset.id).first()
    ...
    fun withoutWalletListsOnlySharedBanners() = runBlocking(Dispatchers.IO) {
        val banners = BannersRequest(database.bannersDao())(null, asset.id).first()
