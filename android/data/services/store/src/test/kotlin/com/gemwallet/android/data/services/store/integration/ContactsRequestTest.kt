...
class ContactsRequestTest {
    ...
    fun listsEveryContactByNameWithAllItsAddresses() = runBlocking(Dispatchers.IO) {
        val contacts = ContactsRequest(database.contactsDao())().first()
    ...
    fun listsTheChainsAddressesByContactName() = runBlocking(Dispatchers.IO) {
        val recipients = ContactRecipientsRequest(database.contactsDao())(Chain.Ethereum).first()
    ...
    fun readsOneContactWithItsAddresses() = runBlocking(Dispatchers.IO) {
        val request = ContactRequest(database.contactsDao())
