...
data class AddNodeUIState(val chain: Chain? = null, val errorText: String = "", val checks: List<NodeCheckRowUIModel> = emptyList(), val warning: ListItemModel? = null, val buttonState: ButtonState = ButtonState.Disabled)

data class NodeCheckRowUIModel(val model: ListItemModel, val isInSync: Boolean? = null)

internal fun GemAddNodeSession.uiState(context: Context): AddNodeUIState {
    val state = viewState()
