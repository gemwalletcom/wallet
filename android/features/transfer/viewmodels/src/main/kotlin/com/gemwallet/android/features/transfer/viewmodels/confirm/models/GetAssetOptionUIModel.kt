...
data class GetAssetOptionUIModel(val action: GetAssetAction, val model: ListItemModel)

internal fun acquireOptions(context: Context, request: AcquireAssetRequest): List<GetAssetOptionUIModel> = listOf(
    GetAssetOptionUIModel(
        action = GetAssetAction.Buy(request.buyAmount),
        model = ListItemModel(
    ...
    GetAssetOptionUIModel(
        action = GetAssetAction.Swap(request.swapPayAssetId),
        model = ListItemModel(
