...
class AmountValidationTest {
    ...
    fun `insufficient balance error uses asset symbol`() {
        val error = assertThrows(AmountError.InsufficientBalance::class.java) {
            AmountValidation.validate(asset, Crypto(BigInteger("200000000")), BigInteger("100000000"), BigInteger.ZERO)
        }
    ...
    fun `validate passes when amount equals balance`() {
        AmountValidation.validate(asset, Crypto(BigInteger("100000000")), BigInteger("100000000"), BigInteger.ZERO)
    }

    @Test
    fun `validate throws ZeroAmount for zero`() {
        assertThrows(AmountError.ZeroAmount::class.java) {
            AmountValidation.validate(asset, Crypto(BigInteger.ZERO), BigInteger("100000000"), BigInteger.ZERO)
        }
    }
    ...
    fun `validate throws MinimumValue when below minimum`() {
        assertThrows(AmountError.MinimumValue::class.java) {
            AmountValidation.validate(asset, Crypto(BigInteger("500000")), BigInteger("100000000"), BigInteger("1000000"))
        }
    }
