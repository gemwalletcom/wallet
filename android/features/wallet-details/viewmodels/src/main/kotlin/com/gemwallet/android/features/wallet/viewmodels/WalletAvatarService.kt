...
class WalletAvatarService @Inject constructor(
    ...
    suspend fun setEmoji(walletId: WalletId, emoji: String, backgroundColor: Int) =
        walletService.setAvatarImage(walletId.id, EmojiAvatarRenderer.render(context, emoji, backgroundColor))

    suspend fun setNftImage(walletId: WalletId, url: String) = walletService.setAvatarImageUrl(walletId.id, url)

    suspend fun reset(walletId: WalletId) = walletService.removeAvatarImage(walletId.id)
}
