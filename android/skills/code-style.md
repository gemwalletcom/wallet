...
- **Dispatchers in view models**: a view model never names `Dispatchers.IO` or `Dispatchers.Default`. It takes `@param:IoDispatcher private val ioDispatcher: CoroutineDispatcher` (`data/services/gemstone/src/main/kotlin/com/gemwallet/android/data/services/gemstone/di/DispatchersModule.kt`) and hands it to every `flowOn`, `launch`, and `withContext` (`features/confirm/viewmodels/src/main/kotlin/com/gemwallet/android/features/confirm/viewmodels/ConfirmViewModel.kt`). A hardcoded dispatcher is work no test scheduler owns: `stateIn(viewModelScope)` hops back to `Dispatchers.Main` from a real thread, so an assertion races it and a coroutine that outlives the test crashes a later one after `resetMain()`. An open base class takes it as `protected val` and its subclasses thread it through (`features/asset_select/viewmodels/src/main/kotlin/com/gemwallet/android/features/asset_select/viewmodels/BaseAssetSelectViewModel.kt`); the qualifier lives in `:data:services:gemstone`, so a feature module that had no reason to depend on it declares one. Coordinators and services keep naming the dispatcher at their own boundary — see [Never call Core from the main thread](../../docs/ARCHITECTURE.md#never-call-core-from-the-main-thread)
