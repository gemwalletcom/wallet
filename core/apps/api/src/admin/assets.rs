...
pub async fn add_asset(_permission: PermissionAdminWrite, asset_id: Json<AssetId>, cacher: &State<CacherClient>, stream_producer: &State<StreamProducer>) -> Result<ApiResponse<AssetId>, ApiError> {
...
pub async fn add_asset_associations(_permission: PermissionAdminWrite, payload: Json<FetchAssetAssociationsPayload>, stream_producer: &State<StreamProducer>) -> Result<ApiResponse<FetchAssetAssociationsPayload>, ApiError> {
...
pub async fn fetch_asset_status(_permission: PermissionAdminWrite, asset_id: Json<AssetId>, database: &State<Database>, stream_producer: &State<StreamProducer>) -> Result<ApiResponse<AssetId>, ApiError> {
    let asset_id = asset_id.into_inner();
