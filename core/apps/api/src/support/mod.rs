...
pub async fn post_support_image(
    ...
    Ok(client.send_image(&device.device_row, image.data, image.file_name, image.content_type).await?.into())
}

#[post("/devices/support/action", format = "json", data = "<action>")]
pub async fn post_support_action(device: AuthenticatedDevice, action: DeviceJson<SupportAction>, client: &State<SupportApiClient>) -> Result<ApiResponse<bool>, ApiError> {
    Ok(client.run_action(&device.device_row, action.into_inner()).await?.into())
}
