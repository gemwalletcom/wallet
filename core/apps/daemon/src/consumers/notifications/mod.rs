...
use streamer::{ConsumerStatusReporter, InAppNotificationPayload, NotificationsFailedPayload, NotificationsPayload, QueueName, ShutdownReceiver, StreamProducer, StreamProducerConfig, StreamReader, run_consumer};
...
pub async fn run(settings: Settings, shutdown_rx: ShutdownReceiver, reporter: Arc<dyn ConsumerStatusReporter>) -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    futures::future::try_join_all(vec![
        tokio::spawn(run_notification_consumer(settings.clone(), QueueName::NotificationsPriceAlerts, shutdown_rx.clone(), reporter.clone())),
        tokio::spawn(run_notification_consumer(settings.clone(), QueueName::NotificationsTransactions, shutdown_rx.clone(), reporter.clone())),
        tokio::spawn(run_notification_consumer(settings.clone(), QueueName::NotificationsObservers, shutdown_rx.clone(), reporter.clone())),
        tokio::spawn(run_notification_consumer(settings.clone(), QueueName::NotificationsSupport, shutdown_rx.clone(), reporter.clone())),
        tokio::spawn(run_notification_consumer(settings.clone(), QueueName::NotificationsRewards, shutdown_rx.clone(), reporter.clone())),
        tokio::spawn(run_notification_consumer(settings.clone(), QueueName::NotificationsFiatPurchase, shutdown_rx.clone(), reporter.clone())),
        tokio::spawn(run_notifications_failed_consumer(settings.clone(), database.clone(), QueueName::NotificationsFailed, shutdown_rx.clone(), reporter.clone())),
...
async fn run_notification_consumer(settings: Arc<Settings>, queue: QueueName, shutdown_rx: ShutdownReceiver, reporter: Arc<dyn ConsumerStatusReporter>) -> Result<(), Box<dyn Error + Send + Sync>> {
...
async fn run_notifications_failed_consumer(settings: Arc<Settings>, database: Arc<Database>, queue: QueueName, shutdown_rx: ShutdownReceiver, reporter: Arc<dyn ConsumerStatusReporter>) -> Result<(), Box<dyn Error + Send + Sync>> {
    let name = queue.to_string();
    let stream_reader = StreamReader::new(reader_config(&settings.rabbitmq, name.clone()), &shutdown_rx).await?.ok_or("shutdown during connect")?;
...
async fn run_in_app_notifications_consumer(settings: Arc<Settings>, database: Arc<Database>, shutdown_rx: ShutdownReceiver, reporter: Arc<dyn ConsumerStatusReporter>) -> Result<(), Box<dyn Error + Send + Sync>> {
