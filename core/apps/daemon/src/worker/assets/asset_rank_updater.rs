...
impl AssetRankUpdater {
    ...
    pub async fn update_suspicious_assets(&self) -> Result<usize, Box<dyn Error + Send + Sync>> {
        ...
        let spam_count = self.database.assets()?.update_assets(spam, vec![AssetUpdate::Rank(AssetRank::Spam.threshold()), AssetUpdate::IsEnabled(false)])?;
        let fraudulent_count = self.database.assets()?.update_assets(fraudulent, vec![AssetUpdate::Rank(AssetRank::Fraudulent.threshold()), AssetUpdate::IsEnabled(false)])?;
