...
impl ProxyResponse {
    ...
    pub(crate) fn with_proxy_headers(mut self, request_id: &str, latency: Duration, cache_status: CacheStatus) -> Self {
        self.headers.insert(X_REQUEST_ID, HeaderValue::from_str(request_id).unwrap_or_else(|_| HeaderValue::from_static("unknown")));
        self.headers.insert(X_CACHE, cache_status.value());
        self.headers
            .insert(X_UPSTREAM_LATENCY, HeaderValue::from_str(&format!("{}ms", latency.as_millis())).unwrap_or_else(|_| HeaderValue::from_static("0ms")));
