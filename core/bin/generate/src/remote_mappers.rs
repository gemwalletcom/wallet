...
impl Config {
    ...
    pub(crate) fn mock_override(&self, record: &str, field: &str) -> Option<&str> {
...
pub struct Generator {
    ...
    pub(crate) mocked: Vec<RemoteType>,
...
impl Generator {
    pub fn load(root: &Path) -> Self {
        Self::parse(Config::from_yaml(CONFIG), &root.join(PRIMITIVES_SOURCE), &root.join(GEMSTONE_SOURCE))
    }

    pub fn parse(config: Config, primitives: &Path, gemstone: &Path) -> Self {
...
fn core_types(gemstone: &Path) -> (BTreeMap<String, RemoteType>, BTreeMap<String, String>, BTreeSet<String>) {
    ...
    for path in source_files(gemstone) {
        ...
        while let Some(line) = lines.next() {
            ...
            let Some((keyword, name)) = declaration(line.trim()) else {
                let attribute = line.trim().starts_with("#[");
                exported = attribute && (exported || ["uniffi::Record", "uniffi::Enum", "uniffi::remote(Record)", "uniffi::remote(Enum)"].iter().any(|derive| line.contains(derive)));
                continue;
            };
            if !std::mem::take(&mut exported) {
                continue;
            }
            let body = body(&mut lines);
...
pub(crate) fn unwrap(type_name: &str) -> (&str, Wrapper) {
