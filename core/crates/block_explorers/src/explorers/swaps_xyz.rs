...
impl BlockExplorer for SwapsXyzScan {
    ...
    fn get_swap_tx_url(&self, input: &ExplorerInput) -> String {
        let hash = byte_serialize(input.hash.as_bytes()).collect::<String>();
...
mod tests {
    ...
    fn test_swap_transaction_url() {
        ...
        assert_eq!(SwapsXyzScan.get_swap_tx_url(&ExplorerInput::from(hash)), format!("https://scan.swaps.xyz/transactions?search={hash}"));
        assert_eq!(SwapsXyzScan.get_swap_tx_url(&ExplorerInput::from("hash+/=")), "https://scan.swaps.xyz/transactions?search=hash%2B%2F%3D");
    }
}
