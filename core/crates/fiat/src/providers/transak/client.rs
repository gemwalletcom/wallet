use super::{
    ...
    models::{Asset, Country, CreateWidgetUrlRequest, CreateWidgetUrlResponse, Data, FiatCurrency, QuoteQuery, RefreshTokenRequest, Response, TokenResponse, TransakQuote, TransakResponse},
...
impl TransakClient {
    ...
    pub fn new(client: ReqwestClient, gateway: ReqwestClient, api_key: String, api_secret: String, referrer_domain: String, access_token_cacher: Arc<dyn AccessTokenCacher>) -> Self {
        TransakClient {
            client,
            gateway,
            api_key,
    ...
    pub async fn get_sell_quote(&self, symbol: String, fiat_currency: String, fiat_amount: f64, network: String) -> Result<TransakQuote, Box<dyn std::error::Error + Send + Sync>> {
        ...
        let sell_quote = self.get_quote("sell", symbol.clone(), fiat_currency.clone(), None, Some(&buy_quote.crypto_amount.to_string()), network.clone()).await?;
    ...
    pub async fn get_quote(&self, quote_type: &str, symbol: String, fiat_currency: String, fiat_amount: Option<f64>, crypto_amount: Option<&str>, network: String) -> Result<TransakQuote, Box<dyn std::error::Error + Send + Sync>> {
        ...
        self.client.get::<TransakResponse<TransakQuote>>(TransakTarget::Quotes { query }).headers(self.headers()).await?.into()
    }
    ...
    pub async fn redirect_url(&self, quote: TransakQuote, data: &FiatQuoteUrlData) -> Result<String, Box<dyn Error + Send + Sync>> {
        self.create_widget_url(map_widget_params(&self.api_key, &self.referrer_domain, quote, data), &data.ip_address).await
    }
    ...
    async fn refresh_access_token(&self) -> Result<(String, Duration), Box<dyn Error + Send + Sync>> {
        ...
        let response: Data<TokenResponse> = self.client.post(TransakTarget::RefreshToken { api_key: self.api_key.clone() }, &body).headers(headers).await?;
