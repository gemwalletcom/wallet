...
use primitives::{Asset, Chain, FeeRate, TransactionFee, TransactionInputType, TransactionLoadData, TransactionLoadInput, TransactionLoadMetadata, TransactionPreloadInput};
...
impl<C: Client> ChainTransactionLoad for AlgorandProvider<C> {
    ...
    async fn get_transaction_load(&self, _input: TransactionLoadInput) -> Result<TransactionLoadData, Box<dyn Error + Sync + Send>> {
        ...
        Ok(TransactionLoadData {
            fee: TransactionFee::new_from_fee(BigInt::from(params.min_fee), Asset::from_chain(Chain::Algorand)),
            metadata,
        })
    }
