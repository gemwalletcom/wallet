...
impl BscParser {
    fn parse_log(context: &ParseContext<'_>, log: &Log) -> Option<PrimitivesTransaction> {
        if !log.address.eq_ignore_ascii_case(STAKE_HUB_ADDRESS) {
            return None;
        }
...
mod tests {
    use chrono::DateTime;
    use gem_evm::rpc::model::Transaction;
    ...
    fn test_map_smartchain_staking_transactions() {
        let cases = [
            ...
            (
                include_str!("../testdata/transaction_staking_redelegate.json"),
                include_str!("../testdata/transaction_staking_redelegate_receipt.json"),
                TransactionType::StakeRedelegate,
        ...
        for (transaction, receipt, transaction_type, from, to, value) in cases {
            ...
            assert_eq!(staking_transaction.fee_asset_id, AssetId::from_chain(Chain::SmartChain));
        ...
        let mut transaction = load_json_rpc_result::<Transaction>(include_str!("../testdata/transaction_staking_delegate.json"));
        let receipt = load_json_rpc_result(include_str!("../testdata/transaction_staking_delegate_receipt.json"));
        transaction.to = Some("0x1234567890123456789012345678901234567890".to_string());

        assert!(ProtocolParsers::map_transaction_with_parsers(&Chain::SmartChain, &transaction, &receipt, DateTime::default(), &[&BscParser]).is_none());
    }
}
