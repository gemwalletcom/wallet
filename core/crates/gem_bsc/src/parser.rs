...
mod tests {
    use chrono::DateTime;
    use gem_evm::rpc::EthereumMapper;
    ...
    use super::SmartChainStakingParser;
    use crate::stake_hub::STAKE_HUB_ADDRESS;
    ...
    fn test_map_smartchain_staking_transactions() {
        ...
        for (transaction, receipt, transaction_type, from, to, value) in cases {
            ...
            let staking_transaction = map_transaction(Box::new(SmartChainStakingParser), &Chain::SmartChain, &transaction, &receipt);
            ...
            assert_eq!(staking_transaction.contract.as_deref(), Some(STAKE_HUB_ADDRESS));
        ...
        let mapped = EthereumMapper::map_transaction(
            Chain::SmartChain,
            &transaction,
            &receipt,
            &BigUint::from(1735671600u64),
            &[Box::new(SmartChainStakingParser)],
        )
        .unwrap();
        assert_eq!(mapped.transaction_type, TransactionType::StakeDelegate);
        assert_eq!(mapped.from, "0x51eD60604637989d19D29e43c5D94B098A0d1Af7");
        assert_eq!(mapped.to, "0xd34403249B2d82AAdDB14e778422c966265e5Fb5");
        assert_eq!(mapped.contract.as_deref(), Some(STAKE_HUB_ADDRESS));
        assert_eq!(mapped.value, "1000000000000000000");

        let mut transaction = transaction;
        transaction.to = Some("0x1234567890123456789012345678901234567890".to_string());

        assert!(ProtocolParsers::map_transaction(&Chain::SmartChain, &transaction, &receipt, DateTime::default(), &[Box::new(SmartChainStakingParser)]).is_none());
    }
}
