...
use primitives::{Asset, GasPriceType, StakeType, SwapProvider, TransactionFee, TransactionInputType, chain_cosmos::CosmosChain};
...
pub fn calculate_transaction_fee(input_type: &TransactionInputType, chain: CosmosChain, gas_price_type: &GasPriceType) -> Result<TransactionFee, Box<dyn Error + Sync + Send>> {
    ...
    Ok(TransactionFee {
        ...
        fee_asset: Asset::from_chain(chain.as_chain()),
    })
}
