...
use crate::{provider::transaction_state_mapper::map_transaction_status, rpc::EthereumProvider};
use gem_client::Client;

#[cfg(feature = "rpc")]
#[async_trait]
impl<C: Client + Clone> ChainTransactionState for EthereumProvider<C> {
...
mod chain_integration_tests {
    ...
    async fn test_ethereum_get_transaction_status_confirmed() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        assert_eq!(result.changes, vec![TransactionChange::BlockNumber("22820942".to_string()), TransactionChange::NetworkFee(BigInt::from(42850974395536u64))]);
    ...
    async fn test_smartchain_get_transaction_status_confirmed() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        assert_eq!(result.state, TransactionState::Confirmed);
        assert_eq!(result.changes, vec![TransactionChange::BlockNumber("55082355".to_string()), TransactionChange::NetworkFee(BigInt::from(27753700000000u64))]);
