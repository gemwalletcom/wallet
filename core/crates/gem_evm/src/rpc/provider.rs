...
mod tests {
    ...
    async fn test_rpc_only_provider_returns_empty_transactions_and_asset_balances() {
        let client = EthereumClient::mock(EVMChain::OpBNB);
    ...
    async fn test_provider_injection() {
        let ethereum = EthereumProvider::new_rpc_only(EthereumClient::mock(EVMChain::Ethereum));
        ...
        let ethereum = EthereumProvider::new_rpc_only_with_provider(EthereumClient::mock(EVMChain::Ethereum), Box::new(MockChainProvider));
