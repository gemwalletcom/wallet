...
mod tests {
    ...
    fn test_clean() {
        ...
        assert_eq!(Mnemonic::clean(&phrase).unwrap().as_str(), ABANDON_PHRASE);
        assert_eq!(Mnemonic::clean("abandon abandon").unwrap_err(), KeystoreError::invalid_input("mnemonic"));
    }
