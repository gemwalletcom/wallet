...
pub(in crate::rpc::indexer) fn map_raw_transaction(transaction: FastNearTransaction) -> Result<Transaction, Box<dyn Error + Send + Sync>> {
    let created_at = DateTime::<Utc>::from_timestamp_nanos(i64::try_from(transaction.execution_outcome.block_timestamp)?);
    let block_number = transaction.execution_outcome.block_height.to_string();
    let state = transaction.state();
    let fee = transaction.fee().to_string();
    let asset_transfers = map_fungible_token_transfers(&transaction.receipts)?;
    let (transfer, transaction_type, metadata) = if let Some(transfer) = asset_transfers.first().cloned() {
        ...
        let (from, to, actions) = transaction
            ...
            .unwrap_or((&transaction.transaction.signer_id, &transaction.transaction.receiver_id, &transaction.transaction.actions));
    ...
    mapped.block_number = Some(block_number);
    Ok(mapped)
}
