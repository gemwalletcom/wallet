...
pub fn map_token_data_metaplex(chain: Chain, token_id: String, token_info: &TokenInfo, meta: &Metadata, asset_type: AssetType) -> Result<Asset, Box<dyn std::error::Error + Send + Sync>> {
...
pub fn map_token_data_spl_token_2022(chain: Chain, token_id: String, token_info: &TokenInfo) -> Result<Asset, Box<dyn std::error::Error + Send + Sync>> {
    let token_metadata = token_info
        .extensions
        .as_ref()
        .and_then(|extensions| extensions.iter().find_map(|ext| if let Extension::TokenMetadata(token_metadata) = ext { Some(token_metadata.state.clone()) } else { None }))
        .ok_or("no token metadata found")?;
    Ok(Asset::new(AssetId::from_token(chain, &token_id), token_metadata.name, token_metadata.symbol, token_info.decimals, AssetType::SPL2022))
}
...
mod tests {
    ...
    fn test_map_token_spl_token_2022() {
        let result = serde_json::from_str::<JsonRpcResult<ResultTokenInfo>>(include_str!("../../testdata/pyusd_mint.json")).unwrap().result.value.data.parsed.info;
