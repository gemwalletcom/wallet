...
mod tests {
    ...
    fn test_sign_data_blockhash() {
        ...
        *durable_nonce.recent_blockhash_mut() = [7; 32];
        ...
        message
            .instructions
            .insert(0, CompiledInstruction::mock(2, vec![1], ADVANCE_NONCE_ACCOUNT_DISCRIMINANT.to_vec()));
    ...
    fn test_sign_message_rejects_transaction_payloads() {
        ...
        assert_eq!(
            SolanaChainSigner.sign_message(&message, &TEST_PRIVATE_KEY).unwrap_err().to_string(),
            format!("Invalid input: {SIGN_MESSAGE_PAYLOAD_REJECTION}")
        );
    }
}
