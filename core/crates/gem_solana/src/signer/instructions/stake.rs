...
mod tests {
    ...
    fn test_sign_stake() {
        ...
        let stake_account = stake_account::from_blockhash(&Pubkey::from_base58(TEST_PRIVATE_KEY_SOLANA_ADDRESS).unwrap(), &input).unwrap();
        ...
        assert_eq!(transaction.instructions()[1].data, {
            ...
            let authority = Pubkey::from_base58(TEST_PRIVATE_KEY_SOLANA_ADDRESS).unwrap();
        ...
        assert_eq!(account_key(&transaction, 0, 0), Pubkey::from_base58(TEST_RECIPIENT).unwrap());
        assert_eq!(account_key(&transaction, 0, 1), Pubkey::from_base58(TEST_PRIVATE_KEY_SOLANA_ADDRESS).unwrap());
    }
    ...
    fn test_sign_reference_stake() {
        ...
        for (stake_type, value, expected) in [
            (StakeType::Stake(validator), "42", REFERENCE_DELEGATE_STAKE_TX),
            (StakeType::Unstake(delegation.clone()), "0", REFERENCE_DEACTIVATE_STAKE_TX),
            ...
            let result = signer.sign_stake(&input, &private_key).unwrap();
            assert_eq!(base58_transaction(&result[0]), expected);
        }
    }
}
