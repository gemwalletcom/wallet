...
mod tests {
    ...
    fn swap_input(encoded: &str, gas_limit: Option<&str>, priority_fee: u64, unit_price: u64) -> SignerInput {
        ...
        let fee = TransactionFee::new_gas_price_type(GasPriceType::solana(5_000u64, priority_fee, unit_price), 5_000u64.into(), 1u64.into(), Default::default(), AssetId::from_chain(Chain::Solana));
    ...
    fn test_sign_swap_v1_uses_absolute_priority_fee_and_quote_gas_limit() {
        ...
        let result = SolanaChainSigner.sign_swap(&swap_input(&encoded, Some(&gas_limit), 12_345, 999_999), &TEST_PRIVATE_KEY).unwrap();
        ...
        assert_eq!(signed.transaction_config().unwrap().loaded_accounts_data_size_limit, original_config.loaded_accounts_data_size_limit);
    ...
    fn test_sign_swap_rejects_a_missing_compute_unit_price() {
        ...
        assert_eq!(result.unwrap_err().to_string(), "Invalid input: Solana swap transaction has no compute unit price instruction to replace");
    }
}
