use crate::{
    CompiledInstruction, Message, MessageAddressTableLookup, MessageHeader, Pubkey, SignatureBytes, TransactionConfig, VersionedMessageV0, VersionedMessageV1,
...
pub(crate) fn mock_transaction(programs: &[(&str, Vec<u8>)]) -> VersionedTransaction {
    ...
    let instructions = programs
        .iter()
        .enumerate()
        .map(|(index, (_, data))| CompiledInstruction::mock((index + 1) as u8, vec![], data.clone()))
        .collect();
...
pub(crate) fn mock_v0_transaction(
...
pub(crate) fn mock_v1_transaction(signature_count: u8, wallet_index: u8) -> VersionedTransaction {
    ...
    VersionedTransaction::V1 {
        signatures: (0..signature_count)
            .map(|value| {
                if value == wallet_index {
                    SignatureBytes::default()
                } else {
                    SignatureBytes::new([value + 1; 64])
                }
            })
            .collect(),
        message: VersionedMessageV1::mock(
            signature_count,
            account_keys,
            vec![CompiledInstruction::mock(signature_count, vec![0], vec![0xde, 0xad])],
