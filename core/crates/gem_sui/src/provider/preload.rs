...
use primitives::{
    Asset, Chain, FeeRate, GasPriceType, StakeType, TransactionFee, TransactionInputType, TransactionLoadData, TransactionLoadInput, TransactionLoadMetadata,
...
impl SuiClient {
    ...
    async fn estimate_fee(&self, tx_data: &str, gas_price: &GasPriceType) -> Result<TransactionFee, Box<dyn Error + Send + Sync>> {
        ...
        Ok(TransactionFee {
            ...
            options: HashMap::new(),
            fee_asset: Asset::from_chain(Chain::Sui),
        })
    }
