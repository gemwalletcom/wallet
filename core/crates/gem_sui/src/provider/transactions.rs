...
use crate::rpc::{SuiIndexer, SuiProvider};

#[cfg(feature = "rpc")]
#[async_trait]
impl ChainBlockTransactions for SuiProvider {
...
#[cfg(feature = "rpc")]
#[async_trait]
impl<C: Client> ChainTransactions for SuiIndexer<C> {
    async fn get_transactions_by_address(&self, request: TransactionsRequest) -> Result<TransactionsResult, Box<dyn std::error::Error + Sync + Send>> {
        let transactions = self.get_transaction_digests_by_address(&request.address, request.limit).await?;
