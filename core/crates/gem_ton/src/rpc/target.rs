...
impl Target for TonCenterTarget {
    fn path(&self) -> String {
        match self {
            ...
            Self::GetAddressInformation { address } => format!("/api/v3/addressInformation?address={address}"),
            Self::GetWalletInformation { address } => format!("/api/v3/walletInformation?address={address}"),
            Self::SendBoc => "/api/v3/message".to_string(),
            Self::EmulateTonConnect => "/api/emulate/v1/emulateTonConnect".to_string(),
            Self::RunGetMethod => "/api/v3/runGetMethod".to_string(),
