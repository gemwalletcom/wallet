...
use gem_aptos::{AptosClient, MoveOption};
...
impl NameResolver for AptosProvider {
    ...
    async fn resolve(&self, query: &NameQuery, _chain: Chain) -> Result<Option<String>, Box<dyn Error + Send + Sync>> {
        ...
        let request = json!({
            "function": format!("{ANS_ROUTER_ADDRESS}::router::get_target_addr"),
            "type_arguments": [],
            "arguments": [domain, MoveOption { vec: subdomain.into_iter().collect() }],
        });
