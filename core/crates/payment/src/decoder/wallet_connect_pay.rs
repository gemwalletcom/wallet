...
use super::query;
use primitives::payment::{Payment, PaymentLink, WALLET_CONNECT_PAY_HOST};
use primitives::{HTTPS_URL_SCHEME, WALLET_CONNECT_URL_SCHEME, WalletConnectLink};
...
fn from_pairing_uri(url: &Url) -> Option<String> {
    ...
    let payment_url = query::value(&query::parameters(Url::parse(&uri).ok()?.query().unwrap_or_default()), QUERY_PAY)?;
...
fn from_payment_url(url: &Url) -> Option<String> {
    ...
    query::value(&query::parameters(url.query().unwrap_or_default()), QUERY_PAYMENT_ID)
