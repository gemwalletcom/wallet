...
use primitives::{ApprovalData, SignableTransaction, SignableTransactionType, TransferDataOutputType};
...
pub fn map_wallet_rpc(account: &str, wallet_rpc: &WalletRpcAction) -> Result<PaymentAction, WalletConnectPayError> {
    ...
    .map_err(WalletConnectPayError::InvalidRequest)?;
    if let WalletConnectAction::Unsupported { method } = action {
        return Err(WalletConnectPayError::Unsupported(method));
    }
    validator::validate_signer(account, &params, &action)?;
    map_action(with_encoded_solana_output(action))
}

fn map_action(action: WalletConnectAction) -> Result<PaymentAction, WalletConnectPayError> {
    match action {
        ...
        WalletConnectAction::SignTransaction { chain, transaction_type, data } => Ok(PaymentAction::SignTransaction {
            chain,
            transaction: WalletConnectRequestHandler::decode_send_transaction(transaction_type, data).map_err(WalletConnectPayError::InvalidRequest)?,
        }),
        WalletConnectAction::SendTransaction { chain, transaction_type, data } => {
            let transaction = WalletConnectRequestHandler::decode_send_transaction(transaction_type, data).map_err(WalletConnectPayError::InvalidRequest)?;
        ...
        WalletConnectAction::SignAllTransactions { .. } => Err(WalletConnectPayError::Unsupported("signAllTransactions".to_string())),
        WalletConnectAction::ChainOperation { .. } => Err(WalletConnectPayError::Unsupported("chainOperation".to_string())),
        WalletConnectAction::GetAccounts { .. } => Err(WalletConnectPayError::Unsupported("getAccounts".to_string())),
        WalletConnectAction::Unsupported { method } => Err(WalletConnectPayError::Unsupported(method)),
    }
}
...
fn map_approval(transaction: &SignableTransaction) -> Option<ApprovalData> {
    ...
    Some(ApprovalData::new(data.to.clone(), param(APPROVE_SPENDER)?, param(APPROVE_VALUE)?))
}
...
mod tests {
    ...
    fn test_map_wallet_rpc_action_rejects() {
        ...
        assert!(matches!(
            ...
            Err(WalletConnectPayError::InvalidRequest(message)) if message.contains("mismatch")
        ));
        assert!(matches!(
        ...
        assert!(matches!(
            ...
            Err(WalletConnectPayError::Unsupported(method)) if method == "eth_sign"
        ));

        assert!(matches!(
            map_wallet_rpc("invalid-account", &actions[0]),
            Err(WalletConnectPayError::InvalidRequest(message)) if message.contains("Invalid option account")
        ));
    }
}
