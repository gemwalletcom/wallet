...
pub fn normalize(method: &str, params: &Value) -> Result<Value, WalletConnectPayError> {
...
fn typed_data_with_schema(params: &Value) -> Result<Value, WalletConnectPayError> {
...
fn with_domain_schema(typed_data: &Value) -> Result<Value, WalletConnectPayError> {
...
fn insert_domain_schema(typed_data: &Value) -> Result<Value, WalletConnectPayError> {
...
fn domain_schema(domain: &Value) -> Result<Vec<EIP712Type>, WalletConnectPayError> {
...
fn object<'a>(value: &'a Value, name: &str) -> Result<&'a Map<String, Value>, WalletConnectPayError> {
    value.as_object().ok_or_else(|| invalid(format!("Expected {name} object")))
}

fn invalid(message: impl Display) -> WalletConnectPayError {
    WalletConnectPayError::InvalidRequest(message.to_string())
}
