...
pub fn map_quoted_payment(response: PaymentOptionsResponse) -> Result<QuotedPayment, WalletConnectPayError> {
    let payment = response.info.ok_or(WalletConnectPayError::PaymentNotFound)?;
    Ok(QuotedPayment {
...
fn map_expiry(payment: &PaymentInfo) -> Result<DateTime<Utc>, WalletConnectPayError> {
    DateTime::from_timestamp(payment.expires_at, 0).ok_or_else(|| WalletConnectPayError::InvalidRequest("Invalid payment expiry".to_string()))
}
...
mod tests {
    ...
    fn test_map_payment_options_without_info() {
        ...
        assert_eq!(map_quoted_payment(response), Err(WalletConnectPayError::PaymentNotFound));
    }
