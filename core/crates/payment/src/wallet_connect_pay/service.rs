...
impl<C: Client> WalletConnectPayService<C> {
    ...
    pub(super) async fn prepare_payment(&self, quotes: &PaymentQuotes, quote: &PaymentQuote, addresses: &[ChainAddress]) -> Result<PreparedPayment, WalletConnectPayError> {
        ...
        match self.payment_actions(quote).await {
            ...
            Err(WalletConnectPayError::QuoteExpired) => self.get_requoted_actions(quote, addresses).await,
            Err(error) => Err(error),
        }
    }

    async fn get_requoted_actions(&self, expired: &PaymentQuote, addresses: &[ChainAddress]) -> Result<PreparedPayment, WalletConnectPayError> {
        let quotes = match self.payment_options(&expired.payment_id, addresses).await? {
            PaymentOptions::Quotes(quotes) => quotes,
            PaymentOptions::Outcome(_) => return Err(WalletConnectPayError::PaymentExpired),
        };
    ...
    pub(super) async fn confirm_payment(&self, quote: &PaymentQuote, action_results: Vec<String>) -> Result<PaymentOutcome, WalletConnectPayError> {
    ...
    pub(super) async fn payment_options(&self, payment_id: &str, addresses: &[ChainAddress]) -> Result<PaymentOptions, WalletConnectPayError> {
        ...
        if identifiers.is_empty() {
            return Err(WalletConnectPayError::UnsupportedAccounts);
        }
        ...
        match quoted.status {
            ...
            PaymentStatus::Failed | PaymentStatus::Expired | PaymentStatus::Cancelled => return Err(WalletConnectPayError::PaymentExpired),
        }
        if quoted.expires_at <= Utc::now() {
            return Err(WalletConnectPayError::PaymentExpired);
        }

        let quotes = Self::payment_quotes(payment_id, quoted.options);
        if quotes.is_empty() {
            return Err(WalletConnectPayError::NoPaymentOptions);
        }
    ...
    fn get_quote(quotes: &PaymentQuotes, asset_id: &AssetId) -> Result<PaymentQuote, WalletConnectPayError> {
        quotes
            .quotes
            .iter()
            ...
            .cloned()
            .ok_or(WalletConnectPayError::NoPaymentOptions)
    }

    async fn payment_actions(&self, quote: &PaymentQuote) -> Result<Vec<PaymentAction>, WalletConnectPayError> {
        let option = Self::option(quote)?;
        let actions = self.wallet_rpc_actions(&quote.payment_id, &option).await?;
        if actions.is_empty() {
            return Err(WalletConnectPayError::InvalidRequest("Payment option has no executable actions".to_string()));
        }

        actions.iter().map(|action| map_wallet_rpc(&option.account, action)).collect()
    }

    pub(super) async fn cancel_payment(&self, payment_id: &str) -> Result<(), WalletConnectPayError> {
        self.client.cancel(payment_id).await
    }

    pub(super) async fn get_payment_status(&self, payment_id: &str) -> Result<PaymentOutcome, WalletConnectPayError> {
    ...
    async fn wallet_rpc_actions(&self, payment_id: &str, option: &PaymentOption) -> Result<Vec<WalletRpcAction>, WalletConnectPayError> {
    ...
    fn option(quote: &PaymentQuote) -> Result<PaymentOption, WalletConnectPayError> {
        serde_json::from_str(&quote.provider_data).map_err(|error| WalletConnectPayError::InvalidRequest(error.to_string()))
    }
}
...
mod tests {
    ...
    async fn test_get_payment_options_rejects_unpayable() {
        ...
        assert_eq!(failed.payment_options("pay_123", &addresses()).await, Err(WalletConnectPayError::PaymentExpired));

        let expired = service_with_response(|response| {
            response["info"]["expiresAt"] = serde_json::json!(1);
        });
        assert_eq!(expired.payment_options("pay_123", &addresses()).await, Err(WalletConnectPayError::PaymentExpired));

        let no_options = service_with_response(|response| {
            far_future(response);
            response["options"] = serde_json::json!([]);
        });
        assert_eq!(no_options.payment_options("pay_123", &addresses()).await, Err(WalletConnectPayError::NoPaymentOptions));
        ...
        assert_eq!(
            ...
            Err(WalletConnectPayError::UnsupportedAccounts)
        );
    }
