...
use crate::{HTTPS_URL_SCHEME, WalletConnectLink};
...
pub fn decode(uri: &str) -> Result<Payment> {
    ...
    Ok(Payment::Link(PaymentLink::WalletConnectPay(payment_id)))
}
...
mod tests {
    ...
    fn payment_id(uri: &str) -> Option<String> {
        link_id(decode(uri).ok())
    }

    fn link_id(payment: Option<Payment>) -> Option<String> {
        match payment? {
            Payment::Link(PaymentLink::WalletConnectPay(payment_id)) => Some(payment_id),
            _ => None,
        }
    }
    ...
    fn test_decode_pairing_uri() {
        ...
        assert_eq!(payment_id("wc:abc@2?pay=http%3A%2F%2Fpay.walletconnect.com%2F%3Fpid%3Dpay_1"), None);
