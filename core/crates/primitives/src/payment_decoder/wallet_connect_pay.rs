...
const WALLET_CONNECT_PAY_HOST: &str = "pay.walletconnect.com";
...
pub fn parse(uri: &str) -> Option<String> {
...
mod tests {
    ...
    #[test]
    fn test_parse() {
        assert_eq!(parse("https://pay.walletconnect.com/?pid=pay_1"), Some("pay_1".to_string()));
        assert_eq!(parse("https://pay.walletconnect.com/pay_1"), Some("pay_1".to_string()));
        assert_eq!(parse("https://staging.pay.walletconnect.com/?pid=pay_1"), Some("pay_1".to_string()));
        assert_eq!(parse("http://pay.walletconnect.com/?pid=pay_1"), None);

        assert_eq!(parse("https://pay.walletconnect.com"), None);
        assert_eq!(parse("https://pay.walletconnect.com/terms"), None);
        assert_eq!(parse("https://pay.walletconnect.com/?pid=checkout"), None);
        assert_eq!(parse("https://notpay.walletconnect.com/pay_1"), None);
        assert_eq!(parse("https://pay.walletconnect.com.attacker.io/pay_1"), None);
        assert_eq!(parse("https://gemwallet.com/tokens/bitcoin"), None);

        assert_eq!(parse("wc:abc@2?pay=https%3A%2F%2Fpay.walletconnect.com%2F%3Fpid%3Dpay_1"), Some("pay_1".to_string()));
        assert_eq!(parse("wc:abc@2?pay=https://pay.walletconnect.com/?pid=pay_1"), Some("pay_1".to_string()));
        assert_eq!(parse("wc:abc@2?pay=https%3A%2F%2Fmerchant.example.com%2Fcheckout"), None);
        assert_eq!(parse("wc:abc@2?pay=http%3A%2F%2Fpay.walletconnect.com%2F%3Fpid%3Dpay_1"), None);
        assert_eq!(parse("wc:abc@2?relay-protocol=irn&symKey=123"), None);

        assert_eq!(
            parse("gem://wc?uri=wc%3Aabc%402%3Fpay%3Dhttps%253A%252F%252Fpay.walletconnect.com%252F%253Fpid%253Dpay_1"),
            Some("pay_1".to_string())
        );
        assert_eq!(parse("gem://wc?uri=wc%3Atopic%402%3Frelay-protocol%3Dirn%26symKey%3Dabc"), None);
        assert_eq!(parse("gem://wc?sessionTopic=abc"), None);
    }
