...
pub(crate) trait TagStore {
    ...
    fn get_list_tags(&mut self) -> Result<Vec<TagRow>, diesel::result::Error>;
...
impl TagStore for DatabaseClient {
    ...
    fn get_tag(&mut self, _tag_id: &str) -> Result<Option<TagRow>, diesel::result::Error> {
        use crate::schema::tags::dsl::*;
        tags.find(_tag_id).select(TagRow::as_select()).first(&mut self.connection).optional()
    }

    fn get_list_tags(&mut self) -> Result<Vec<TagRow>, diesel::result::Error> {
        use crate::schema::tags::dsl::*;
        tags.filter(list_id.is_not_null()).order(id.asc()).select(TagRow::as_select()).load(&mut self.connection)
    }
