...
impl UsernamesStore for DatabaseClient {
    fn get_username(&mut self, lookup: UsernameLookup) -> Result<UsernameRow, diesel::result::Error> {
        ...
        match lookup {
            UsernameLookup::Username(username) => dsl::usernames.filter(lower(dsl::username).eq(username.to_lowercase())).select(UsernameRow::as_select()).first(&mut self.connection),
            UsernameLookup::WalletId(wallet_id) => dsl::usernames.filter(dsl::wallet_id.eq(wallet_id)).select(UsernameRow::as_select()).first(&mut self.connection),
        }
    }

    fn create_username(&mut self, username: NewUsernameRow) -> Result<UsernameRow, diesel::result::Error> {
        use crate::schema::usernames::dsl;
        diesel::insert_into(dsl::usernames).values(&username).returning(UsernameRow::as_returning()).get_result(&mut self.connection)
    }
