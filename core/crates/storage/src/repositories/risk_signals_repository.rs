...
pub trait RiskSignalsRepository {
    ...
    fn count_unique_referrers_for_device_model_pattern(&mut self, device_model: &str, device_platform: Platform, device_locale: &str, since: NaiveDateTime) -> Result<i64, DatabaseError>;
...
impl RiskSignalsRepository for DatabaseClient {
    ...
    fn get_matching_risk_signals(&mut self, fingerprint: &str, ip_address: &str, ip_isp: &str, device_model: &str, device_id: i32, since: NaiveDateTime) -> Result<Vec<RiskSignalRow>, DatabaseError> {
        Ok(RiskSignalsStore::get_matching_risk_signals(self, fingerprint, ip_address, ip_isp, device_model, device_id, since)?)
    }
    ...
    fn count_unique_referrers_for_device_model_pattern(&mut self, device_model: &str, device_platform: Platform, device_locale: &str, since: NaiveDateTime) -> Result<i64, DatabaseError> {
        Ok(RiskSignalsStore::count_unique_referrers_for_device_model_pattern(self, device_model, device_platform, device_locale, since)?)
    }
    ...
    fn count_unique_countries_for_referrer(&mut self, username: &str, since: NaiveDateTime) -> Result<i64, DatabaseError> {
        Ok(RiskSignalsStore::count_unique_countries_for_referrer(self, username, since)?)
    }

    fn count_unique_devices_for_referrer(&mut self, username: &str, since: NaiveDateTime) -> Result<i64, DatabaseError> {
        Ok(RiskSignalsStore::count_unique_devices_for_referrer(self, username, since)?)
    }
}
