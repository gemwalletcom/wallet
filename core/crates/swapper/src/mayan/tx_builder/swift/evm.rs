...
mod tests {
    ...
    fn test_swift_random_key_with_memo() {
        let random = super::super::swift_random_key(&MayanSwiftQuote::mock()).unwrap();
    ...
    fn test_swift_order() {
        ...
        let order = swift_order(&quote, &MayanSwiftQuote::mock(), 2, quote_destination_address(&quote), None).unwrap();
    ...
    fn test_hypercore_custom_payload() {
        let mut route = MayanSwiftQuote::mock();
