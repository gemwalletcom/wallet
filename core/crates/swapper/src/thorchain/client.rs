    ...
    C: Client + Clone + Send + Sync + Debug + 'static,
{
    ...
    pub async fn get_quote(&self, from_asset: THORChainAsset, to_asset: THORChainAsset, value: String, streaming_interval: i64, streaming_quantity: i64, affiliate: String, affiliate_bps: i64) -> Result<QuoteSwapResponse, SwapperError> {
        ...
        self.client.get_or_error::<_, ErrorResponse>(ThorChainTarget::Quote { network: self.network, request }).await.map_err(SwapperError::from)
    }

    pub async fn get_inbound_addresses(&self) -> Result<Vec<InboundAddress>, SwapperError> {
        self.client.get(ThorChainTarget::InboundAddresses { network: self.network }).await.map_err(SwapperError::from)
    }
