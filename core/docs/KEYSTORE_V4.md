...
- Web needs a `Keystore` backend over IndexedDB. The trait is synchronous (UniFFI callers are), so the browser backend needs an in-memory mirror or an async variant; once that is settled, hoist the remaining backend-neutral logic (payload validation, import idempotency, meta production) behind a record-IO trait so a backend only supplies storage; `FileKeystore` fails closed on non-unix targets (`file_io.rs`) rather than writing world-readable secrets.
