...
**Needs no decision, but is wide.** Nine of the twelve file-scope `Gem*Service()` holders are gone. The pattern is settled — copy it rather than reinventing:
...
Done: `GemPaymentService`, `GemAssetConfigService` in `TransactionsRequestFilter`, `GemAddressService` (Android), `GemAddressService` (iOS `Chain+GemstonePrimitives`), `GemApplicationMetadataService`, `GemTransferService` in `ConfirmParams`, `GemDeeplinkService` (iOS `URLParser`), `GemTransferService` in iOS `TransferData+`, plus a dead `Chain.defaultAssetRank`.

The three left were each measured:
...
| `ext/Chain.kt` (Android) | 21 errors over 11 files | **Attempted and reverted — the size was never the problem.** The "~52 callers" figure was a bad grep; the compiler says twenty-one. What stops it is `Migration_71_72`, a Room migration `object` that calls `chain.asset()` to seed asset rows at database open, where there is no graph to inject from — making it a class means changing the Room builder and an `androidTest` that needs a device to run. The rest is ordinary: eight Compose files (a fourth `CompositionLocal`), an entity mapper, one view model. Also note the cache: `chainAsset` is a `by lazy` map over every chain, so any version that takes a service must keep caching or it decodes JSON per call. |
| `Chain+GemstonePrimitives` (iOS) | 21 sites; `assetConfig` half remains | **Half done.** The `addressService` holder is gone: `isValidAddress`/`checksumAddress` now take the service, threaded through `AddressTextValidator` → `AddressInputViewModel` → `PaymentDestinationBuilder` → the contacts and recipient view models → `ViewModelFactory`/`NavigationHandler`, with a new `@Entry addressService` for the two navigation views that build view models from the environment. Twenty-one sites, seven test suites, no behaviour change. What is left in the file is `assetConfig`, which is the frozen-table case below — a different question, not a bigger version of the same one. |

On the two `Chain.asset()` table cases specifically (Android `ext/Chain.kt`, and the `assetConfig` half of iOS `Chain+GemstonePrimitives`): that service is used once, to build an immutable lookup at first access — not reached per call. Threading a dependency through fifty-odd sites so a pure `Chain.asset()` can read a constant makes every caller carry a parameter for nothing at runtime. The rule's stated reason is that a call site reaching for a service is where an app-side variant creeps in, and a frozen table cannot drift. Worth deciding whether the rule should carve this out before spending the change. **Method note, learned three times over:** grep counts on property names are useless here — `.feeAsset` matched 381 lines and was really twenty sites; `ConfirmParams` was "59 files" and was twelve; `Chain.asset()` was "~52 callers" and was twenty-one. Make the change, let the compiler count, and revert if it lands somewhere a dependency cannot go.
