...
What is left of the stack is the connection lifecycle: iOS `HyperliquidObserverService` (actor, one wallet at a time, cancels its observe task) and Android `HyperliquidObserverService` (foreground flag, `distinctUntilChangedBy`, two collectors) still decide *when* to connect in their own idioms, and `PerpetualNodeService`/`toWebSocketUrl` still build the `"ws"` URL app-side. Fold those next: the URL rule belongs to Core node config, and the connect/disconnect decision can follow `GemStreamService`'s shape.
