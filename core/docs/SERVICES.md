...
**Needs no decision, but is wide — and the old count here was wrong.** This section used to say "twelve file-scope holders" and then "the three left". Both numbers were bookkeeping of specific *named* sites, not a survey: `GemPaymentService`, `GemDeeplinkService` and `GemApplicationMetadataService` were each marked done after one site was converted while other holders of the same service stayed. A real sweep (`= Gem*Service()` outside tests and DI modules) finds **21 on iOS and 7 on Android**. Do not trust a count in this file that was not produced by that grep.
...
Two things make this cheaper than it looks. Most of these services are **already injected somewhere in the same feature** — `SwapSceneViewModel` held `GemSwapQuoteService` while two extensions it called constructed their own; `ConfirmViewModel` on Android held `GemFeeService` while iOS did not. So the work is usually threading an existing dependency one or two levels, not introducing one. And the errors arrive **one module at a time**: grep every construction site of the type you just changed before recompiling, or you will pay a full build per call site.

Converted so far: `GemPaymentService` (in `ConfirmParams`), `GemAssetConfigService` in `TransactionsRequestFilter`, `GemAddressService` on both platforms, `GemApplicationMetadataService`, `GemTransferService` in `ConfirmParams` and iOS `TransferData+`, `GemDeeplinkService` in iOS `URLParser`, `GemFeeService` on iOS, `GemSwapQuoteService` on iOS, plus a dead `Chain.defaultAssetRank` and a dead `GasPriceType.custom(base:gasPrice:)`.
...
Still open, and each needs its own pass:
...
| `ext/Chain.kt` (Android) and the `assetConfig` half of `Chain+GemstonePrimitives` (iOS) | 21 sites on Android | **Attempted and reverted on Android — the size was never the problem.** What stops it is `Migration_71_72`, a Room migration `object` that calls `chain.asset()` to seed asset rows at database open, where there is no graph to inject from — making it a class means changing the Room builder and an `androidTest` that needs a device. Also note the cache: `chainAsset` is a `by lazy` map over every chain, so any version that takes a service must keep caching or it decodes JSON per call. See the frozen-table question below before spending this one. |
| `GemAmountService` (iOS) | `AmountValueValidator`, `AmountDataProvidable`, `GemTransferAmount+` | Three holders of one service in the transfer amount path; `AmountDataProvidable` is a protocol, so the service has to reach conformers. |
| `GemAssetConfigService` (iOS) | `SelectAssetFlow`, `AddAssetInput`, `AssetScore+`, `AssetProperties+`, `AssetBasic+`, `Chain+` | Six holders. The four `*+GemstonePrimitives` ones are the frozen-table case; the two in `Features/Assets` are ordinary. |
| `GemChainService` | iOS `Chain+WalletConnect`, `NetworkSelectorViewModel`; Android `SelectFilterChain.kt` | `NetworkSelectorViewModel` uses a defaulted property rather than file scope, which is the same problem one level in. |
| `GemPaymentService` (iOS) | `GemPayment+`, `PaymentDestinationBuilder` | `PaymentDestinationBuilder` already takes `addressService`; this is the second hop on the same type. |
| `GemDeeplinkService` (Android) | `RootRoute.kt`, `AssetDetailsMenu.kt` | Compose files — use the `CompositionLocal` pattern. |
| `GemTransferService` (iOS) | `LocalKeystore` | One site, in the keystore. |
| `GemAssetConfigService` (Android) | `AssetDefaults.kt`, `ConfirmErrorInfo.kt` | Frozen-table case and one Compose file. |
| `GemPaymentService` (Android) | `PaymentDestination.kt` | Mirror of the iOS one. |

On the `Chain.asset()` / `assetConfig` table cases specifically: that service is used once, to build an immutable lookup at first access — not reached per call. Threading a dependency through fifty-odd sites so a pure `Chain.asset()` can read a constant makes every caller carry a parameter for nothing at runtime. The rule's stated reason is that a call site reaching for a service is where an app-side variant creeps in, and a frozen table cannot drift. Worth deciding whether the rule should carve this out before spending the change. **Method note, learned four times over:** grep counts on property names are useless here — `.feeAsset` matched 381 lines and was really twenty sites; `ConfirmParams` was "59 files" and was twelve; `Chain.asset()` was "~52 callers" and was twenty-one; and the holder count in this very section was wrong in both directions. Make the change, let the compiler count, and revert if it lands somewhere a dependency cannot go.
