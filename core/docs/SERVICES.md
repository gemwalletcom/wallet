...
- **The Android perpetual chart was always showing an error.** `GetPerpetualChartDataImpl` passed `period.string` where `ChartPeriod` crosses the FFI as JSON, so `serde_json::from_str` rejected the bare token and every chart load fell through to `StateViewType.Error`. A regression from `3bb03e34`, found while moving the candle rules — which also means the merge rule was unreachable on Android the whole time. Fixed in `ebf469e`.
...
- **Android's swap error collapse was destroying the information retryability needs.** Its `SwapError` mapping folded `NoAvailableProvider` and `InvalidRoute` into a single `NoQuote`, so the app could not tell a transient failure from a permanent one for that pair. The quote and transfer phases carry the `SwapperException` itself now.
