...
| 4 | Hyperliquid streaming | [`HyperliquidObserverService`](../../ios/Packages/GemstoneServices/Sources/Perpetual/HyperliquidObserverService.swift) (134) | [`HyperliquidObserverService`](../../android/data/services/gemstone/src/main/kotlin/com/gemwallet/android/data/services/gemstone/perpetual/HyperliquidObserverService.kt) (113) + `PerpetualStreamConnection` | Read both: the "120 lines of shared dance" is not there. The common part is a three-case event switch (`connected` / `message` / `disconnected`) that already forwards straight to `GemPerpetualStreamService`; everything around it is genuine platform concurrency — an actor with a `Task` on iOS, a `CoroutineScope` with `collectLatest` on Android — plus lifecycle. **Fixed while reading it**: iOS recorded `currentWallet` *before* asking for the connection, so one failed lookup stranded the stream permanently for that wallet (the `already connected` guard then short-circuited every retry, and `disconnect` returns early with no task to clear). The socket, stream service and chart service are injected now, so that path has a test. **Left**: iOS has no foreground gate — Android connects only while foregrounded and re-emits on return, iOS connects on `setup(for:)` and stays connected. That is the same question as T6, and it is a lifecycle decision, not a rule to move. |
