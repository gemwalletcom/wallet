...
**Row 3 (confirm) is closed.** Metadata, simulation assets and the recent-activity read moved first; then `FeeAssetProvider` became `GemConfirmService.fee_assets`, which reads balances through `GemBalanceService` and applies `rules::selectable_fee_assets`. Core returns the qualifying `AssetId`s and each app loads its own row data for them, so Core owns *which* fee assets qualify without modelling the rendering aggregate — the objection that had this parked. `FeeAssetProvider` and `FeeAssetProvidable` are **deleted** on iOS — `ConfirmTransferInputProvider` calls Core directly and reads the selected fee asset straight from `AssetStore`. `GemFeeAsset` carries asset, balance and price, so no app-side code assembles the row: `FeeAssetItem` now holds exactly those three fields instead of a whole `AssetData`, which removes the `.with(asset:)` default-metadata path the old objection was about. Android's `ChainFeeAssetProvider` keeps observing the store for change triggers but asks Core on each emission, which deleted its duplicate `feeAssetIds` set and `hasAvailable()` filter.

**The preload orchestration is Core's too.** `GemConfirmService.preload` composes load → metadata → fee asset → available value → amount calculation and returns `GemConfirmPreload { confirm_data, metadata, fee_asset, amount }`, where `amount` is a `GemTransferAmountResult` enum so an unaffordable transfer still returns a renderable preload rather than failing the whole call. That deleted iOS's `TransferTransactionProvider` (its error mapping moved into `ConfirmTransferInputProvider`, which is now a pure mapper) and Android's `CalculateTransferAmount` case, `CalculateTransferAmountImpl` and its test — both platforms were building `GemTransferAmountInput` and calling `GemAmountService.calculate` themselves, on inputs Core already had. Android's `GemTransferAmountException.toConfirmError` moved from `data/coordinators` to `gemcore` beside `ConfirmError`, where both callers can reach it. Covered by `test_preload_reports_the_amount_error_instead_of_failing_the_whole_preload` in Core (mutation-checked) plus mapping tests on each platform.

**Wallet and session services.** Both were already thin forwarders to `GemWalletService`/`GemWalletSessionService`; what was left app-side is platform glue (keystore secret reads, `preferences.reload()` after the last wallet is deleted, the temporary v3 migration) plus one real rule that had drifted. The rewards entry in settings was `wallets.contains(multicoin)` on iOS and `isEmpty() || any(multicoin)` on Android — Android showed rewards with no wallets, iOS hid them. It is `GemWalletSessionService.shows_rewards` now. **Android's reading won, and deliberately:** `rewards stay available while no wallets are loaded` is an existing test pinning the empty case as anti-flicker, so the divergence was a decision, not drift — Core adopted it and iOS gained it. `WalletSessionManageable.hasMulticoinWallet` is gone.

Worth knowing: both platforms' old tests mocked the provider that called `wallet.account(for:)`, so no fixture had an account for its own transfer chain. Routing through Core surfaced that immediately — if a confirm fixture starts failing with "account not found", give the mock wallet an account for `data.chain`. Both layers are mutation-checked: `test_a_fee_asset_with_no_available_balance_is_not_selectable` in Core, `keepsOnlyTheFeeAssetsCoreSelected` on Android, and `loadsAssetDataForTheFeeAssetsCoreSelected` on iOS.

**Closed — do not re-open, and do not re-add from a survey.** Rows 1, 2, 4, 5 and 7 were each checked against the code and found already done or misdescribed. Two carry a warning worth keeping: iOS *does* have a foreground gate for the perpetual stream (`handleScenePhase`), so row 4's "Android only" claim was wrong; and what remains in Android's `SystemAuthPolicy` is platform (`BiometricManager` constants and an Android-only auth queue), so exporting it would put a rule in Core with one consumer. `InitialAuthErrorTest` pins the 500 ms retry delay against Core's, so those two cannot drift silently.
...
**Needs no decision, but is wide — and the old count here was wrong.** This section used to say "twelve file-scope holders" and then "the three left". Both numbers were bookkeeping of specific *named* sites, not a survey: `GemPaymentService`, `GemDeeplinkService` and `GemApplicationMetadataService` were each marked done after one site was converted while other holders of the same service stayed. A real sweep (`= Gem*Service()` outside tests and DI modules) found **21 on iOS and 7 on Android**. Do not trust a count in this file that was not produced by that grep.

**Where it stands now: 6 left on iOS, 2 on Android**, down from 21 and 7. Six of those eight are the frozen `assetConfig` table; the other two are `AddressFormatter` and the `BiometryAuthenticationService` default, both explained in the table below. **Every holder that was not blocked by a documented obstacle is gone.**

The pattern is settled — copy it rather than reinventing:

- a view model, coordinator or other injected class → constructor injection
- a Compose scene → one instance provided at `MainActivity` through `CompositionLocalProvider`, read as `LocalAddressService.current` (see `LocalAddressService`, `LocalChainService`, `LocalDeeplinkService`, `LocalAssetConfigService`, `LocalApplicationMetadataService`, `LocalTransferService`)
- a plain helper inside a Compose file → an explicit parameter, because a `CompositionLocal` cannot be read outside a `@Composable` (`selectFilterChain` is a `LazyListScope` extension, so it takes the service; its two composable callers read the local)
- a value type or namespace of statics → make it an instance that is handed the service (`URLParser`)
- a value-type extension → the method takes the service; the caller that can hold one passes it down
- a SwiftUI view that builds a view model from the environment → an `@Entry` in `ios/Gem/Types/Environment.swift` sourced from `AppResolver.main.services`

Three things make this cheaper than it looks. Most of these services are **already injected somewhere in the same feature** — `SwapSceneViewModel` held `GemSwapQuoteService` while two extensions it called constructed their own; `ViewModelFactory` already held `feeService` while `NetworkFeeCustomViewModel` reached for its own. So the work is usually threading an existing dependency one or two levels, not introducing one. The errors arrive **one module at a time**: grep every construction site of the type you just changed before recompiling, or you pay a full build per call site. And on Android, `androidTest` is **not** covered by `testGoogleDebugUnitTest` — `PaymentDestination`'s instrumented callers only surfaced under `assembleGoogleDebugAndroidTest`, so run that too.

Converted: `GemPaymentService` (both platforms), `GemAssetConfigService` in `TransactionsRequestFilter` and Android's `ConfirmErrorInfo`, `GemAddressService` (both), `GemApplicationMetadataService` (both), `GemTransferService` (`ConfirmParams`, iOS `TransferData+`, iOS `LocalKeystore`), `GemDeeplinkService` (both), `GemFeeService` (iOS), `GemSwapQuoteService` (iOS), `GemAmountService` (iOS), `GemChainService` (iOS WalletConnect and `NetworkSelectorViewModel`, Android `SelectFilterChain`), `GemSecurityService` (Android), `GemAssetConfigService` in iOS `SelectAssetFlow`/`AddAssetInput`, plus a dead `Chain.defaultAssetRank` and a dead `GasPriceType.custom(base:gasPrice:)`.

`GemFeeService` is the worked example of the whole shape: the two extension files held their own instance, `NetworkFeeCustomViewModel` → `NetworkFeeSceneViewModel` → `ConfirmTransferSceneViewModel`/`TransactionSceneViewModel` → `ViewModelFactory` already had the graph, and the factory already held `feeService` — the holders existed purely because nobody threaded the last two hops. One of its two rules, `customGasPrice`, turned out to have no caller on either platform; the Core function stays because `confirm/rules.rs` uses it internally, but the iOS wrapper was dead and is gone.

Still open:

| Holder | Where | Why it is its own pass |
|---|---|---|
...
| `assetConfig`: Android `ext/Chain.kt` and `AssetDefaults.kt`, iOS `Chain+`, `AssetScore+`, `AssetProperties+`, `AssetBasic+` | 21 sites on Android | The frozen-table case — see below. Android's is additionally blocked by `Migration_71_72`, a Room migration `object` that calls `chain.asset()` at database open, where there is no graph to inject from; making it a class means changing the Room builder and an `androidTest` that needs a device. Note the cache: `chainAsset` is a `by lazy` map over every chain, so any version taking a service must keep caching or it decodes JSON per call. |
| `GemSecurityService` (iOS) | `BiometryAuthenticationService` | A *defaulted* service parameter rather than file scope. Making it required pushes the default into `SecurityViewModel` and `LockSceneViewModel`, which also default-construct the whole `BiometryAuthenticationService` — so it is a lock-manager pass, not a one-liner. |

On the `Chain.asset()` / `assetConfig` table cases specifically: that service is used once, to build an immutable lookup at first access — not reached per call. Threading a dependency through fifty-odd sites so a pure `Chain.asset()` can read a constant makes every caller carry a parameter for nothing at runtime. The rule's stated reason is that a call site reaching for a service is where an app-side variant creeps in, and a frozen table cannot drift. Worth deciding whether the rule should carve this out before spending the change. **Method note, learned five times over:** grep counts on property names are useless here — `.feeAsset` matched 381 lines and was really twenty sites; `ConfirmParams` was "59 files" and was twelve; `Chain.asset()` was "~52 callers" and was twenty-one; and the holder count in this very section was wrong in both directions. Make the change, let the compiler count, and revert if it lands somewhere a dependency cannot go — `AddressFormatter` did, and is recorded above rather than forced.

`NetworkSelectorViewModel` was recorded here as blocked and turned out not to be, which is worth remembering as a failure mode of this file. The claim was that `SelectableListAdoptable` requires `init(state:selectedItems:selectionType:)` so a service parameter breaks conformance. True — but that requirement existed *only* so the protocol could offer an `init(state:)` shorthand, and exactly three conformers used it. Deleting both `init` requirements and giving those three default arguments removed the constraint entirely. When a protocol requirement blocks an injection, check what the requirement is *for* before recording it as an obstacle.

One more shape worth knowing, met in `AssetsFilterViewModel`: storing a service costs a value type its synthesised `Equatable`. Where the remaining fields are the value's real identity (`type`, `chainsFilter`, `hasBalance`) a hand-written `==` over them is correct and small — the service is a stateless dependency, not state. Where the type has many fields, or equality drives something subtle, that is the point to stop and reconsider, as `CopyTypeViewModel`/`WalletViewModel` did under `AddressFormatter`.
...
| # | What | iOS | Android | The difference |
|---|---|---|---|---|
| T3 | Swap quote debounce | `SwapScene.swift` debounce 250 ms | `RequestSwapQuotes.QUOTE_DEBOUNCE_MS` 500 ms | The 30 s refresh interval is now `GemSwapQuoteService.refresh_interval_milliseconds`, read by both. The debounce is still two numbers and wants a decision, not a coin flip — 250 ms is snappier, 500 ms halves a fan-out across swap providers. Same question as the 250/500 ms name-resolution debounce. **Not drift:** Android stopping the refresh loop after a failed quote is deliberate and pinned by `quote errors do not schedule automatic retries`. Whether iOS should also stop retrying is the open question; do not silently delete the Android break. |
| T5 | In-app polling | two `onTimer` sites, both Core-driven now | fiat ticker, Core-driven | The row described four hardcoded iOS timers (positions, charts, asset, activity); only two `onTimer` sites exist, and both read their interval from Core — swap from `GemSwapQuoteService`, fiat from `GemFiatService`. `PRICES_UPDATE_INTERVAL_SECONDS` is not unused either: Core reads it in `prices_outdated`, and Android exposes it through `PerpetualConfig`. What is left is the design question the row was really about — whether a screen should poll at all on top of a live socket — which is a decision, not a missing call. |
| T7 | Live price subscription | `PriceUpdater.swift`, called from the asset screen and both swap legs | now also subscribed from both swap legs | The user-visible half is fixed: Android's swap screen never subscribed its pair, so fiat values only moved if some other screen had subscribed those assets first. What is left is the original intent — Core deciding *when* prices are subscribed rather than each screen asking — which would then delete `PriceUpdater.swift`. That is a design change, not a missing call. |
| T12 | App-start ordering | `OnstartService` + `RootSceneViewModel` | `SyncService.kt` | `GemAppStartService.run()` now ends with the device sync, so neither app bolts one on: Android's `SyncService` is just `run()`, and iOS dropped the separate task in `RootSceneViewModel.setup`. Still per-platform: the v3 keystore migration runs from `OnstartService` on iOS and `MainViewModel` on Android — left alone deliberately, the migration is temporary. iOS still gates wallet setup on unlock and Android does not, which is a lifecycle decision. |

**T8 is closed.** Android does get `clear_markets` and the markets sync, through `SyncPerpetualsImpl`. The real defect was that `ObservePerpetualWallet` ANDed its own `isPerpetualEnabled()` with `showPerpetuals(wallet)`, which already ANDs that flag inside Core; the flag is now only the change trigger and Core decides.

Still worth knowing before touching these areas:

- **The reconnect cap was a decision, not a default.** Android's 30 s was set deliberately (`d1cdb74`) so the price stream resumes within half a minute; iOS's 60 s was the untouched import default. Core took 30 s — do not "restore" 60 s.
...
- **Stake push notifications go to different places.** iOS `case .stake: break` selects the wallet tab behind a TODO; Android opens the asset. A navigation decision, still open.
- **Dead typeshare models are gone.** `PushNotificationAsset`, `PushNotificationSwapAsset`, `PushNotificationReward`, `PushNotificationWalletAsset` and `PushNotificationTransaction` had no app consumer — all five are used only inside Rust — so their `#[typeshare]` attributes are dropped and `just generate-models` removed 77 lines of generated model across the two platforms. `PushNotificationTypes` keeps its attribute; it is used. Note the gotcha if you do this again: a `#[typeshare(skip)]` on a *field* stops compiling once the struct attribute is removed, so it has to go with it. **An earlier version of this entry also named `Transaction`: that was wrong — `com.wallet.core.primitives.Transaction` is used across Android (`TransactionExtended`, `NotificationNavigation`, test fixtures). Do not drop its attribute.**
- **Same button, two strings.** iOS says "Use Minimum Amount" (`swap_use_minimum_amount`); Android says "Minimum amount" (`stake_minimum_amount`), borrowed from staking. Product call on new copy.
...
A `Gem*Service()` in a field initialiser or at file scope is a second instance of something the graph already owns, and it is where app-side variants creep back in. The full sweep, the settled patterns and the remaining blocked cases are in **What is left** above — do not keep a second count here.
