...
| 3 | Confirm | [`ConfirmService`](../../ios/Features/Transfer/Sources/Services/ConfirmService.swift) + `ConfirmSimulationService`, `ConfirmTransferInputProvider`, `FeeAssetProvider`, `TransferTransactionProvider` | `coordinators/confirm/*` (`BuildConfirmPropertiesImpl`, `ChainFeeAssetProvider`, `ConfirmTransactionImpl`, `GetFeeAssetsImpl`), `ConfirmViewModel` | The seam below. **Metadata is done**: `GemConfirmService.metadata` owns which assets the screen needs and rejects a missing balance with `BalanceMissing`, over `GemBalanceService`/`GemPriceService` rather than their stores; `TransferMetadataProvider` is now a mapper onto it. What is left: `FeeAssetProvider` and `ConfirmSimulationService` both read `AssetStore.getAssetData`, which returns the app-side `AssetData` aggregate (asset + balance + price) that Core does not model — that aggregate has to exist in Core before either can move. Also: `ConfirmService` holds `RecentActivityStore` — a table Core does not own, read from a confirm screen. Move that read behind the service that owns it. |
