...
| Transactions | `TransactionsViewModel` over GRDB requests | `GetTransactions`/`GetTransaction` over `GemstoneTransactionStore` | the row title is `GemTransactionFormatter.title` on both platforms; still open — `GemTransactionsService` answers the point reads, and the counterparty address, the value sign and the swap pair follow the title into the formatter. Only the observed list stays platform-side |
...
The websocket URL rule went with it: `GemNodeService.websocket_node_url(chain)` swaps the scheme and appends `"ws"` once, which deleted iOS `PerpetualNodeService` and Android's `toWebSocketUrl`. What is left of the stack is the connection lifecycle: iOS `HyperliquidObserverService` (actor, one wallet at a time, cancels its observe task) and Android `HyperliquidObserverService` (foreground flag, `distinctUntilChangedBy`, two collectors) still decide *when* to connect in their own idioms. Fold that next, following `GemStreamService`'s shape.
...
| --- | --- | --- | --- |
| ~~`PerpetualRepository` (+`Impl`)~~ | 0 | **gone** | the six observed reads and the search-priority branch are `GemstonePerpetualStore`, which the perpetual cases hold; the autoclose screen asks one case (`GetPerpetualPositionByAsset`) instead of chaining two repository reads. Its `androidTest` suites went with it — they did not compile (`FakePerpetualRepository` still implemented methods the interface had dropped) and instrumented tests do not run in CI; the search filter is covered by a JVM test of the adapter instead |
| ~~`WalletConnectorService`~~ (was `BridgesRepository`) | 0 | **gone** | `WalletConnectCoordinator` owns the client initialization and the merged event stream, and implements `IsWalletConnectEnabled`, `PairWalletConnect`, `GetWalletConnections`, `DisconnectWalletConnection`, `ApproveWalletConnection`, `ApproveWalletConnectAuthentication` and `RespondWalletConnectRequest`. The `WalletConnectClient` abstraction and its models moved to `gemcore` `application/bridge/`, so the Reown and no-op modules no longer depend on `data:repositories` |
| ~~`WalletsRepository` (+`Impl`)~~ | 0 | **gone** | only two of its methods had callers left — the wallet list and one wallet — and both are observed reads. They are the `GetWallets` and `GetWallet` cases over `GemstoneWalletStore`; every write was already `GemWalletService`'s |
| ~~`AssetsRepository`~~ | 0 | **gone** | its reads are cases now: the point reads (`GetAssetInfo`, `GetAssetTokenInfo`, `GetAssetById`, `GetAssetLinks`, `GetAssetMarket`) hold `GemstoneAssetStore`, and the lists hold `WalletAssetsCoordinator`, which owns the one shared current-wallet flow the repository used to cache. `sync` and the widget list are `SyncBalances` and `GetWidgetAssets`; `searchToken` was already `SearchTokensCase` |
| ~~`SessionRepository` (+`Impl`)~~ | 0 | **gone** | `SessionCoordinator` owns the session state — the observed session, the current currency — and implements the six cases the app asks for: `GetSession`, `GetCurrentWallet`, `GetCurrentWalletId`, `GetCurrentCurrency`, `SetCurrentCurrency`, `SetCurrentWallet`, `ClearSession`. It reads through `GemstoneWalletSessionStore` and `GemstoneWalletStore` and writes the current wallet through `GemWalletSessionService` |

  The module is `data/services/gemstone` now, the name all three sides share: Core's `services/<name>/store.rs`, iOS's `Packages/GemstoneServices/Sources/Stores/`, Android's `data/services/gemstone/.../stores/`. Every file in it implements a Core foreign trait over the platform — the `Gemstone*Store` adapters in `stores/`, then `GemDevicePlatform`, `GemFileStore`, `GemStreamConnection`, `GemNotificationPermissions` and `GemPerpetualStreamConnection` in the folders iOS puts them in — plus the long-lived observer services (`DeviceObserverService`, `StreamObserverService`, `HyperliquidObserverService`) and the platform query services Core does not own (`AssetsSearchService`, `RecentAssetsService`).
