...
- **Dead typeshare models.** `PushNotificationAsset`, `PushNotificationSwapAsset`, `PushNotificationReward`, `PushNotificationWalletAsset` and `PushNotificationTransaction` have no app consumer — all five are used only inside Rust, so dropping `#[typeshare]` deletes four generated models per platform and changes no behaviour. Needs `just generate-models` on both. **The earlier entry here also named `Transaction`: that is wrong — `com.wallet.core.primitives.Transaction` is used across Android (`TransactionExtended`, `NotificationNavigation`, test fixtures). Do not drop its attribute.**
