...
**Where it stands now: 8 left on iOS, 2 on Android.** Six of those ten are the two deferred cases below (`AddressFormatter`, and the frozen `assetConfig` table); the rest are listed in the table.
...
Converted: `GemPaymentService` (both platforms), `GemAssetConfigService` in `TransactionsRequestFilter` and Android's `ConfirmErrorInfo`, `GemAddressService` (both), `GemApplicationMetadataService` (both), `GemTransferService` (`ConfirmParams`, iOS `TransferData+`, iOS `LocalKeystore`), `GemDeeplinkService` (both), `GemFeeService` (iOS), `GemSwapQuoteService` (iOS), `GemAmountService` (iOS), `GemChainService` (iOS WalletConnect, Android `SelectFilterChain`), `GemSecurityService` (Android), plus a dead `Chain.defaultAssetRank` and a dead `GasPriceType.custom(base:gasPrice:)`.
...
| `GemAssetConfigService` (iOS) | `SelectAssetFlow`, `AddAssetInput` | Ordinary, not the frozen table — two files in `Features/Assets`. |
