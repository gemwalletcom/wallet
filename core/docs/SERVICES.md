...
| `Chain+GemstonePrimitives` (iOS) | mirrors the above | Same table. iOS has no Room migration, so it may well be tractable on its own — worth measuring with the compiler rather than a grep. |
