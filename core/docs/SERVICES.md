...
| `AssetsRepository` | 41 | reads that forward to `GemstoneAssetStore`, plus `sync`, `searchToken`, `updateBalances` and the widget list | the observed reads moved to the adapter and the write half is gone: `add`, `linkAssetToWallet` and the two insert helpers had no caller left — `GemBalanceService.set_assets_enabled` already syncs the asset, adds the balance row, sets visibility and subscribes the price stream. What is left is turning the 41 callers into cases over `GemAssetsService` |
| `SessionRepository` (+`Impl`) | 107 | the `session()` StateFlow, plus forwards to `GemWalletSessionService` and `GemPreferencesService` | it holds the two store adapters now, not `SessionDao` or `WalletsRepository`. Last to go, in slices: `GetCurrentWallet`/`SetCurrentWallet`/`ObserveSession` cases over `GemWalletSessionService`, currency over `GemPreferencesService` |
