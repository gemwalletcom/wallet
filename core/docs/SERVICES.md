...
- **A rewards push with missing or malformed data navigated nowhere on Android.** `parseNotificationData` decoded the reward payload only to discard it, so an absent or bad payload was indistinguishable from an unrecognised notification and the referral screen never opened. Core returns the destination whether or not the payload decodes. Fixed in `c31c3f17`.
