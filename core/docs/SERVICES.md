...
| [`GemBannerService`](../gemstone/src/services/banner/mod.rs) | [`GemBannerStore`](../gemstone/src/services/banner/store.rs) | [Swift](../../ios/Packages/GemstoneServices/Sources/Stores/BannerStore.swift) | [Kotlin](../../android/data/services/gemstone/src/main/kotlin/com/gemwallet/android/data/services/gemstone/stores/BannerStore.kt) | Banner state rules, plus `banner_content` — which asset field each line names (`asset.name` for a title, `asset.symbol` and the chain's `network_name` for a description), whether the account-activation description has a fee to show, and the icon per event ([rules](../gemstone/src/services/banner/rules.rs)); the apps keep only the localized templates and the image assets |
