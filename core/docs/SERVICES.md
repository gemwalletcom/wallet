...
| [`ChainServiceFactory`](../../ios/Packages/GemstoneServices/Sources/Gateway/ChainServiceFactory.swift), `ChainService`, `ChainServiceable`, `GatewayChainService` + two mocks | a six-file per-chain indirection whose only outside callers are the two chain-settings view models asking a node for its status | delete the layer: Core's `GemGateway.get_node_status(chain, url)` already builds its own provider for that url, so the view models take `GatewayService` and call `nodeStatus(chain:url:)`. Everything else on `ChainServiceable` is already on `GatewayService` with a `chain:` argument and has no other caller |
| [`TransactionStateTracker`](../../ios/Packages/GemstoneServices/Sources/TransactionState/TransactionStateTracker.swift) | fire-and-forget `Task` wrappers around `GemTransactionStateService`, plus `addNotificationTransaction` which calls Core and then tracks | delete: the callers own their `Task`, and the call-then-track pair belongs in Core's `add_notification_transaction` |
