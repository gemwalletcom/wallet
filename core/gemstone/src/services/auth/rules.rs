...
mod tests {
    ...
    fn test_auth_account_uses_ethereum() {
        assert_eq!(
            auth_account(&Wallet::mock_with_accounts(vec![Account::mock(Chain::Bitcoin, "bitcoin-address"), Account::mock(Chain::Ethereum, "ethereum-address")])).map(|account| account.address.as_str()),
