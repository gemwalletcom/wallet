...
pub trait GemStakeStore: Send + Sync {
    async fn get_validators(&self, asset_id: AssetId, provider_type: StakeProviderType) -> Result<Vec<DelegationValidator>, GemStakeError>;
    async fn save_validators(&self, validators: Vec<DelegationValidator>) -> Result<(), GemStakeError>;
    async fn get_delegation_ids(&self, wallet_id: WalletId, asset_id: AssetId, provider_type: StakeProviderType) -> Result<Vec<String>, GemStakeError>;
