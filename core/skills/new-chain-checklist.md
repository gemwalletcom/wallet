...
2. **Crate**: create `crates/gem_<chain>` (auto-included by the `crates/gem_*` workspace glob) with feature flags mirroring the family pattern: `rpc`, `signer`, `testkit`, `chain_integration_tests`.
   ...
   - `signer/`: a `<Chain>Signer` implementing the `EvmSigner` seam when transaction formats differ — only the overridden operations, everything else stays in `EvmChainSigner` (for Tempo: TIP20 transfers and swaps as type `0x76` transactions with a concrete `fee_token`).
