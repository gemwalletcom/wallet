...
Repo-wide architectural choices that are not obvious from the code, with the rationale. Understand the rationale before changing one. Service, state, and FFI decisions live in [Architecture § 13](ARCHITECTURE.md#13-shapes-that-were-tried-and-reverted). Code-style exceptions belong in the platform skills, not here.
