...
WalletConnect Pay links resolve through the gateway in one `load` per screen. `/options` lists the merchant, the price and one quote per wallet asset; EVM token ids are checksummed on the way in because the gateway sends them lowercase. `/fetch` answers what the selected quote asks the wallet to do: a coin is paid by sending the transaction the gateway built; a token by signing a Permit2 or `ReceiveWithAuthorization` typed-data transfer that the gateway relays, so a token payment has no network fee unless the token still needs an approval, in which case the wallet sends the approval first, only to the contract that verifies the permit, pays its gas, and reports the approval hash and the signature in one `/confirm`. Quotes are re-selected by asset id, never by option id: the gateway issues new option ids on every `/options` call and rejects the previous ones, so an option id is only valid for the `/confirm` that follows its own `/fetch`. The built transaction carries the quote's deadline, so a late signature fails on chain instead of being credited and the app keeps no expiry timer of its own. A link whose payment is already final does not open a screen: `load` fails with `PaymentError::Status` and each app shows the status with its own localized text.

A payment is its own transaction input. `load` answers `GemPaymentLoad::Sign` with the transfer data already built: the payment service ensures the asset and builds `TransactionInputType::Payment { asset, invoice, extra }`, whose `invoice` holds the link, the merchant, the price when the rail quotes one and every quote; the quote being paid is the one whose asset is the transfer's asset. Core answers the confirm's title, its price header, its merchant recipient row and its pay-with row from that input, so the screen renders a payment the way it renders swap data. Switching the asset is a load option like a fee priority: `GemConfirmLoadOptions.asset_id` makes the confirm session re-select the quote through the payment service and swap the transfer it holds before it preloads, and every load the session answers carries the transfer it was loaded for.

When `/fetch` refuses a quote until identity data is collected, `load` answers `GemPaymentLoad::Verify` with the form URL; only that error opens the form, the `collectData` URLs in `/options` are not a signal. From a scanned link the app shows the form and handles the link again once it completes. From the confirm screen the session builds the transfer from that quote with `PaymentInvoice.verification` set, skips its preload, keeps the screen not executable, and the app shows a verification row in place of the fee that opens the form; completing it reloads the same asset, which now lands on a signable transfer.

Results are reported inside `execute`, one per gateway action in order: the hash of a coin transfer, the signature of a token permit, or the approve hash then the signature when the token still needed an approval. Succeeded and processing answer nothing; any other status comes back as the result's warning, a `GemErrorText` the app localizes, so the app toasts only a refusal. A token payment has no hash of its own until the gateway's relayer sends it, so a reported one is recorded in activity right away with the payment id as its hash and `TransactionPaymentMetadata { link, merchant }`, and the transaction tracker asks the gateway's `/status` for such records instead of the chain: succeeded swaps the hash for the relayer's transaction id and confirms the record, a final failure fails it, anything else keeps polling. The approve, when there was one, is tracked on chain like any token approval. Solana Pay transaction links answer the same `Sign` with an invoice that quotes nothing, so they keep the plain confirm. The confirm session owns no gateway call of its own; it delegates every one to the payment service.
