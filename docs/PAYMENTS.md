...
WalletConnect Pay links resolve through the gateway in one `load` per screen: `/options` lists the merchant and one quote per wallet asset (EVM token ids are checksummed on the way in; the gateway sends them lowercase), `/fetch` answers what the selected quote asks the wallet to do: a coin is paid by sending the transaction; a token by signing a Permit2 typed-data transfer that the gateway relays, so a token payment has no network fee unless the token still needs an approval, in which case the wallet sends the approval first (only to the contract that verifies the permit), pays only its gas, and reports both the approval hash and the signature in one `/confirm`; and the gateway answers `/fetch` with an identity-data error when the payer still has to fill in the merchant's form. Only that error opens the web form; `collectData` in `/options` stays after the form is done and is not a signal. Quotes are re-selected by asset id, never by option id: the gateway issues new option ids on every `/options` call and rejects the previous ones, so the option id is only valid for the `/confirm` that follows its own `/fetch`. Every quote carries `expiresAt`, and the built transaction's router deadline is that same instant, so a late signature fails on chain instead of being credited; the app does not keep its own expiry timer. A link whose payment is already final does not open a screen: `load` fails with `PaymentError::Status`, and each app shows the status with its own localized text.

A payment is its own transaction input: `load` answers `GemPaymentLoad::Sign` with the transfer data already built, so neither app resolves an asset or maps a transaction; the payment service ensures the asset and builds `TransactionInputType::Payment { asset, invoice, extra }`, whose `invoice` holds the link, the merchant, the price when the rail quotes one, and every option, signed and preloaded like an encoded generic transaction; the option being paid is the one whose asset is the transfer's asset. Core answers the confirm's title, its price header and its merchant recipient row from that input, so the screen renders a payment the way it renders swap data, without calling the payment service; switching an option is a load option like a fee priority: `GemConfirmLoadOptions.asset_id` makes the confirm session re-select through the payment service and swap the transfer it holds before it preloads, and when the gateway refuses the selected option until identity data is collected, the payment service builds the transfer from that quote with `PaymentInvoice.verification` set, the session skips the preload for it, the screen is not executable, and the app shows a verification row in place of the fee that opens the form; completing the form reloads, the session asks the gateway again for a payment that still carries a verification, and the reload lands on a signable transfer; the link flow keeps its form before the first confirm and handles its link again after it; every screen the session answers names the transfer it was loaded for, and the results are reported inside `execute`, with the transfer that was executed, one per gateway action in order (the hash of a coin transfer, the signature of a token permit, the approve hash then the signature when the token still needed an approval): succeeded and processing answer nothing, any other status comes back as the result's warning, a `GemErrorText` the app localizes, so the app toasts only a refusal; the confirm's own sent toast and the activity list already cover success. A token payment has no hash of its own until the gateway's relayer sends it, so a reported one is recorded in activity right away with the payment id as its hash and `TransactionPaymentMetadata { link, merchant }`, and the transaction tracker asks the gateway's `/status` for such records instead of the chain: succeeded swaps the hash for the relayer's transaction id and confirms the record, a final failure fails it, anything else keeps polling; the approve, when there was one, is tracked on chain like any token approval. Solana Pay transaction links answer the same `Sign` with an invoice that quotes nothing, so they keep the plain confirm. Confirm Core has no payment code.
