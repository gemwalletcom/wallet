...
8. **Reuse existing refresh and cache policies.** Avoid duplicate requests, unnecessary writes, and new polling timers. Bound caches and invalidate derived values when their inputs change. For swap, reuse route hints under the [Swapper contract](SWAPPER.md); still request live quotes for the current inputs.
