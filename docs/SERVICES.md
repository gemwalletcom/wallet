...
- **Fee rates arrive in display order.** `GemConfirmData.fee_rates` is sorted by Core (normal, then fast), so iOS's `FeeRateViewModel: Comparable` / `FeePriority.rank` sort is gone and Android, which never sorted, shows the same order.
