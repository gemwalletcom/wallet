...
| Stake payloads as typed FFI | `StakeType`, `Delegation`, `DelegationBase`, `RedelegateData` and `Price` still cross as JSON strings (`json_bridge.rs`; 22 iOS and 12 Android `.json()` / `toJson` / `decodeJson` sites in the stake and amount flows). The generator needs two things first: tuple-variant enums (`Stake(DelegationValidator)` — UniFFI renders them as `case staking(StakeChain)` / Kotlin `v1`, so the primitives enum keeps its shape) with mapper emission for the single-payload variants, and a big-integer field mapper for TypeShare twins, whose app-to-Core direction has to parse the twin's `String` (`DelegationBase.balance`). | Parse loudly: a throwing `map()` on iOS and `requireNotNull` on Android for that one field kind, never a zero default; then the five types go into `remote_types.yml` in one commit and the JSON sites become `map()` / `toGem()`. |
