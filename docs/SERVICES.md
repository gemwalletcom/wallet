...
- A bare number that crosses is not always a rendered one. Reviewed on 2026-09-14: `GemBalanceValue.amount` and `GemPriceUpdate` are written straight to the balance and price tables by both stores, and `GemAssetDetailsInput.price` and `GemPriceAlertSession`'s `input` and `current_price` are inputs Core's own rules consume. None of them picks a separator, a precision or a style, so none of them belongs in the value-and-style contract. A count the screen prints verbatim (`GemNftRow.count_text`, `GemRewardsState.invite_reward_points_text`, `GemAssetDetailsState.price_alerts_count_text`) crosses as its finished text. A bare-number sweep has to split rendered numbers from stored, input and layout ones before it lists anything.
- Both apps pick the network for receiving a collectible through the select-asset flow `GemSelectAssetType::ReceiveCollection`; Android's separate chain picker and Core's `receive_accounts` went on 2026-09-17.
