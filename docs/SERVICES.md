...
| Stake payloads as typed FFI | `StakeType`, `Delegation`, `DelegationBase`, `RedelegateData` and `Price` still cross as JSON strings (`json_bridge.rs`; 22 iOS and 12 Android sites). The TypeShare twins hold `DelegationBase.balance` as `String` while Core holds `BigUint`, so a generated mapper between them would parse a decimal string in the app, which the big-integer rule forbids; that mapper was built and rejected on 2026-09-06. | Do not map: the apps hold Core's `Delegation` (a `BigUInt`/`BigInteger` record) end to end, which means the delegation stores and screens take the uniffi record and the TypeShare twin goes, the way `Wallet` and `Asset` went; `StakeType` follows as a tuple-variant remote enum once its payloads are uniffi types. Until then the JSON bridge is right, because Core does the parsing. |
