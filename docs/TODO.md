...
**D15–D17 reopened on 2026-09-16.** They were closed on the strength of X158, and X158 turned out to be a missing linker flag on the widget target rather than a boundary — `Formatters` and `Validators` can depend on Gemstone. They are ordinary consolidation items again:
...
This reopens **D15–D17**, which § 17 closed on the strength of X158, and puts `PriceChangeColor` in `Components` (§ 17, D19) within reach as well.
