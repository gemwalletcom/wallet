...
- The Network Fee row reads as it does on Confirm, in the user's currency, so the fee the user approved looks the same afterwards; tapping it shows the amount in the network's coin.
