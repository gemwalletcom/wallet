...
- Tapping an address opens its page with the name the user already sees for it, so the address is recognisable: a contact or own wallet name comes first, and a contact shows its picture or initials as on the confirm screen. The page also shows the full address, copied with a tap, the type and the balances; a flagged address shows a "Suspicious address" warning even when it is a contact, and balances are left out when there are none to show.
