...
- A long press pins a wallet into a "Pinned" section above the rest; wallets are ordered Multi-Coin first, then single-network, then watch-only.
- A wallet's screen edits the name (saved as typed), sets an avatar from an emoji or one of its own NFTs, and shows the address with an explorer link for a single-network wallet.
