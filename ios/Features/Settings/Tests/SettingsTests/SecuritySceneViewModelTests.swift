...
struct SecuritySceneViewModelTests {
    ...
    func theSectionsFollowWhetherAuthenticationIsOn() {
        let settings = GemSettingsServiceMock()
        let model = SecuritySceneViewModel.mock(service: BiometryAuthenticationMock(requiresAuthentication: true), settings: settings)
    ...
    func theHideBalanceToggleWritesThroughToPreferences() {
        ...
        let model = SecuritySceneViewModel.mock(preferences: preferences)
