...
struct AmountStakeViewModelTests {
    ...
    func makeTransferData() throws {
        ...
        let stake = try AmountStakeViewModel(asset: .mockBNB(), type: .stake(validators: [validator], recommended: nil)).makeTransferData(value: 100, useMaxAmount: false)
        let unstake = try AmountStakeViewModel(asset: .mockBNB(), type: .unstake(delegation)).makeTransferData(value: 100, useMaxAmount: false)
        let redelegate = try AmountStakeViewModel(asset: .mockBNB(), type: .redelegate(delegation, validators: [validator], recommended: nil)).makeTransferData(value: 100, useMaxAmount: false)
        let withdraw = try AmountStakeViewModel(asset: .mockBNB(), type: .withdraw(delegation)).makeTransferData(value: 100, useMaxAmount: false)
