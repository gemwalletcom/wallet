...
struct GemAPITests {
    ...
    func gemDeviceAPIWalletIdClassifiesWalletScopedTargets() {
        ...
        let transaction = GemDeviceAPI.getTransaction(walletId: walletId, transactionId: TransactionId(chain: .ton, hash: "hash"))
        #expect(transaction.walletId == walletId.id)
        #expect(transaction.path == "/v2/devices/transactions/ton_hash")
