...
struct TransferAmountCalculatorTests {
    ...
    func validateTempoTokenFeeAsset() throws {
        let result = try validateTempoToken(amount: .exact(BigInt(400)), feeBalance: BigInt(1000)).get()
        #expect(result == TransferAmount(value: BigInt(400), networkFee: BigInt(500), useMaxAmount: false))

        let maxResult = try validateTempoToken(amount: .max(BigInt(1000)), feeBalance: BigInt(1000)).get()
        ...
        #expect(validateTempoToken(amount: .exact(BigInt(600)), feeBalance: BigInt(1000)) == .failure(.insufficientBalance(
            tempoToken,
