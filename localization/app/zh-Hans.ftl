# Common

common_cancel = 取消
common_hidden = 已隐藏
common_done = 完成
common_maximum_value = 最大值为 %@
common_minimum_value = 最小值为 %@
common_secret_phrase = 助记词
common_copy = 复制
common_share = 分享
common_continue = 继续
common_continue_anyway = 仍要继续
common_sign_in_with = 使用%@登录
common_provider = 服务商
common_loading = 加载中
common_address = 地址
common_try_again = 重试
common_paste = 粘贴
common_delete = 删除
common_delete_confirmation = 确定要删除%@吗？
common_hide = 隐藏
common_wallet = 钱包
common_show = 显示%@
common_copied = 已复制：%@
common_copied_to_clipboard = 已复制到剪贴板
common_recommended = 推荐
common_all = 全部
common_not_available = 无法使用
common_phrase = 助记词
common_manage = 管理
common_type = 类型
common_no_results_found = 未找到结果
common_no = 否
common_none = 无
common_url = 网址
common_private_key = 私钥
common_latency_in_ms = %d 毫秒
common_open_settings = 打开设置
common_pin = 置顶
common_pinned = 已置顶
common_lists = 列表
common_unpin = 取消置顶
common_pinned_asset = 已置顶：%@
common_unpinned_asset = 已取消置顶：%@
common_skip = 跳过
common_warning = 警告
common_learn_more = 了解更多
common_description = 描述
common_emoji = 表情符号
common_avatar = 头像
common_save = 保存
common_percentage = 百分比
common_details = 详情
common_info = 信息
common_get_started = 开始使用
common_earn = 赚取
common_method = 方法
common_token = 代币
common_expiration = 有效期
common_suspicious_address = 可疑地址
common_suspicious_address_description = 此地址与可疑或恶意行为有关。
common_refresh = 刷新
common_grant_permission = 授予权限
common_required_field = %@为必填项

# Welcome

welcome_title = 欢迎成为Gem家人

# Wallet

wallet_create_new_wallet = 创建新的钱包
wallet_import_existing_wallet = 导入现有钱包
wallet_send = 发送
wallet_receive = 接收
wallet_buy = 购买
wallet_swap = 兑换
wallet_import_title = 导入钱包
wallet_new_title = 新建钱包
wallet_name = 名称
wallet_import_action = 导入
wallet_multicoin = 多币种
wallet_import_address_field = 地址或域名
wallet_manage_token_list = 管理代币
wallet_scan_qr_code = 扫描二维码
wallet_scan = 扫码
wallet_scan_hint = 发送、支付或连接 DApp
wallet_scan_hint_address = 钱包地址
wallet_default_name = 钱包 #%d
wallet_default_name_chain = %@ 钱包 #%d
wallet_copy_address = 复制地址
wallet_watch_tooltip_title = 仅供观察的钱包。这些资金不受您控制。
wallet_watch_empty_state_title = 正在观察这个钱包。
wallet_stake = 质押
wallet_receive_no_memo_required = 无需备注
wallet_receive_no_destination_tag_required = 无需目标标签
wallet_title = 钱包
wallet_add_token_title = 添加代币
wallet_sell = 出售
wallet_receive_collection = 接收 NFT 藏品
wallet_more = 更多
wallet_import_contract_address_field = 合约或代币ID
wallet_import_address_warning = 可以查看此地址的余额和交易，但**不能发送或出售资金**。
wallet_deposit = 充值
wallet_withdraw = 提现
wallet_available_balance = 可用： %@
# Used in the insufficient network fee sheet for the buy option subtitle.
wallet_pay_with_card_or_bank = 使用银行卡或银行付款
# Used in the insufficient network fee sheet for the swap option subtitle.
wallet_from_your_wallet_assets = 使用钱包中的资产
# Used in the insufficient network fee sheet for the receive option subtitle.
wallet_transfer_from_another_wallet = 从另一个钱包转入
wallet_portfolio_title = 投资组合
wallet_import_already_imported_message = 此钱包已导入。

# Wallets

wallets_title = 钱包
wallets_watch = 观察

# Receive

receive_title = 接收%@
receive_warning = 请仅向此地址发送 %2$@ 网络上的 %1$@。

# Buy

buy_title = 购买%@
buy_providers_title = 服务商
buy_rate = 汇率
buy_no_results = 暂无报价

# Transfer

transfer_network_fee = 网络费用
transfer_pay_with = 支付方式
transfer_payment_title = 付款
transfer_memo = 备注
transfer_confirm = 确认
transfer_from = 来自
transfer_to = 至
transfer_max = 最大值
transfer_network = 网络
transfer_recipient_title = 接收地址
transfer_balance = 余额: %@
transfer_title = 转账
transfer_recipient_address_field = 地址或域名
transfer_send_title = 发送
transfer_insufficient_balance = %@余额不足
transfer_insufficient_network_fee_balance = %@ 余额不足，无法支付网络费用。
transfer_approve_title = 授权
transfer_stake_title = 质押
transfer_claim_rewards_title = 领取奖励
transfer_unstake_title = 取消质押
transfer_redelegate_title = 重新质押
transfer_withdraw_title = 赎回
transfer_rewards_title = 奖励
transfer_minimum_amount = 最低金额为%@
transfer_activate_asset_title = 启用资产
transfer_recipient_my_wallets = 我的钱包
transfer_recipient_view_wallets = 查看钱包
transfer_minimum_account_balance = 除非使用全部余额，否则需保留至少 %@ 余额。
transfer_destination_account_activation = 目标地址最低需要 %@。
transfer_smart_contract_title = 智能合约
transfer_other_title = 其他
transfer_maximum_amount = 最高金额为%@
transfer_reserved_fees = 已在余额中预留 %@，用于支付后续网络费用。
transfer_freeze_title = 冻结
transfer_unfreeze_title = 解冻
transfer_review_request = 审查请求
transfer_amount = 金额

# Settings

settings_terms_of_services = 服务条款
settings_privacy_policy = 隐私政策
settings_title = 设置
settings_developer = 开发者
settings_version = 版本
settings_community = 社区
settings_aboutus = 关于我们
settings_networks_title = 网络
settings_networks_source = 来源
settings_networks_explorer = 区块浏览器
settings_currency = 货币
settings_website = 访问网站
settings_security = 安全设置
settings_enable_value = 启用%@
settings_enable_passcode = 启用密码
settings_security_authentication = 身份验证
settings_notifications_title = 通知
settings_language = 语言
settings_help_center = 帮助中心
settings_support = 支持
settings_price_alerts_title = 价格提醒
settings_hide_balance = 隐藏余额
settings_disable_value = 禁用 %@
settings_preferences_title = 偏好设置
settings_preferences_perpetual_default_leverage = 默认杠杆
settings_preferences_perpetual_default_take_profit = 默认止盈
settings_preferences_perpetual_default_stop_loss = 默认止损
settings_appearance_title = 外观
settings_appearance_system = 跟随系统
settings_appearance_light = 浅色
settings_appearance_dark = 深色

# Support

support_state_empty_title = 需要什么帮助？
support_state_empty_description = 有任何问题都可以问我们。我们随时提供帮助。
support_message_placeholder = 消息

# Errors

errors_validation = 验证错误： %@
errors_create_wallet = 创建钱包错误： %@
errors_invalid_address_name = 地址或名称无效
errors_invalid_amount = 数额无效
errors_token_invalid_id = 代币 ID 无效
errors_import_invalid_secret_phrase = 助记词无效
errors_import_invalid_secret_phrase_word = 助记词中包含无效单词：%@
errors_import_invalid_private_key = 无效的私钥
errors_keystore_access = 无法在此设备上访问该钱包的密钥。如持有该钱包的助记词，请删除该钱包并重新导入以恢复访问。
errors_wallet_account_missing = 该钱包没有此网络的账户
errors_invalid_url = 无效的网址
errors_error_occurred = 出错了！
errors_invalid_network_id = 网络 ID 无效
errors_error = 错误
errors_cancelled = 已取消
errors_connections_user_cancelled = 用户已取消
errors_not_supported = 不支持
errors_permissions_not_granted = 未授予权限
errors_unknown = 未知
errors_not_supported_qr = 此设备不支持扫码，只能选图库的二维码图片。
errors_camera_permissions_not_granted = 未获取相机权限。请在设置中开启以扫描二维码。
errors_decoding_qr = 解析二维码失败，请换一个码重试
errors_transfer_error = 转账错误
errors_unable_estimate_network_fee = 现在无法计算网络费用。
errors_invalid_asset_address = 无效的 %@ 地址
errors_dust_threshold = 交易失败，因为金额太小，无法满足%@网络的最低要求（粉尘阈值）。此限制可确保交易金额足以覆盖手续费和处理成本。
errors_swap_no_quote_available = 暂无报价
errors_swap_not_supported_asset = 暂不支持该资产。
errors_connections_no_supported_wallets = 暂无可用钱包
errors_connections_unsupported_chain = 暂不支持该区块链网络
errors_authentication_unavailable = 此设备未设置身份验证。请在设备设置中开启生物识别或密码。
errors_authentication_locked_out = 失败次数过多，请稍后再试。
errors_authentication_failed = 身份验证失败，请重试。
errors_connections_expired = 此连接链接已过期。请获取新的二维码后重试。
errors_connections_not_found = 此连接已不存在。
errors_connections_relay_unavailable = 无法连接 WalletConnect。请检查网络连接后重试。
errors_swap_amount_too_small = 金额太小，请输入更大的数额试试
errors_scan_transaction_malicious_description = 无法完成此交易：目标钱包地址与可疑或恶意行为有关。
errors_scan_transaction_memo_required = %@ 收款地址需要填写目标标签或备注
errors_scan_transaction_malicious_title = 可疑行为
errors_connections_malicious_origin = 此连接来自不可信来源。
errors_no_data_available = 暂无数据
errors_dust_threshold_short = 转账金额过小，被网络视为“粉尘”金额，手续费甚至高于金额本身。
errors_swap_minimum_amount = 最低交易金额为 %@。请输入更高的金额。
errors_perpetual_trigger_price_lower = 触发价格应低于市场价格
errors_perpetual_trigger_price_higher = 触发价格应高于市场价格
errors_unknown_try_again = 发生未知错误，请重试
errors_network_error = 网络错误： %@
errors_network_offline = 网络连接似乎已断开。
errors_no_internet_connection = 无互联网连接
errors_no_service_connection = 无服务连接
errors_balances_activity_outdated = 余额和交易记录可能不是最新的。
errors_payment_status = 付款： %@

# Activity

activity_title = 交易记录
activity_state_empty_title = 交易将在此显示
activity_state_empty_description = 发起第一笔交易
activity_state_empty_search_description = 清空筛选条件后将重新加载交易
activity_state_empty_search_title = 未找到任何交易

# Update App

update_app_title = 有新版本可用！
update_app_description = 应用 %@ 版本现已发布。立即更新，体验最新功能和改进。
update_app_action = 更新
update_app_permission_title = 允许安装
update_app_permission_description = 转到设置并允许从外部来源安装
update_app_permission_open_settings = 前往设置

# Transaction

transaction_recipient = 接收地址
transaction_sender = 发送地址
transaction_date = 日期
transaction_view_on = 在 %@ 上查看
transaction_status = 状态
transaction_estimated_confirmation = 预计确认
transaction_status_confirmed = 成功
transaction_status_pending = 待确认
transaction_status_failed = 失败
transaction_status_reverted = 已回滚
transaction_title_received = 转入
transaction_title_sent = 转出
transaction_swap_again = 再次兑换
transaction_status_completed = 已完成
transaction_status_inprogress = 进行中
transaction_status_refunded = 已退款
transaction_status_expired = 已过期

# Asset

asset_balances = 余额
asset_balances_available = 可用余额
asset_price = 价格
asset_market_cap = 市值
asset_trading_volume = 交易量 (24小时)
asset_total_supply = 总供应量
asset_circulating_supply = 流通供应量
asset_balances_reserved = 系统保留
asset_view_token_on = 在 %@ 上查看代币
asset_view_address_on = 在 %@ 上查看地址
asset_name = 名称
asset_symbol = 代币符号
asset_decimals = 小数位数
asset_token_id = 代币 ID
asset_contract = 合约
asset_state_empty_title = 交易将显示在此处
asset_state_empty_description = 接收、兑换或购买%@
asset_verification_unverified = 未验证
asset_verification_suspicious = 可疑的
asset_verification_warning_title = 了解正在添加的内容
asset_verification_warning_message = 任何人都可以创建代币，请谨防假冒或恶意代币。
asset_buy_asset = 购买%@
asset_get_asset = 获取 %@
asset_add_to_wallet = 添加到钱包
asset_hide_from_wallet = 从钱包隐藏
asset_added_to_wallet = 已添加到钱包
asset_hidden_from_wallet = 已从钱包隐藏
asset_resources = 资源
asset_verification_verified = 已验证
asset_all_time_high = 历史最高点
asset_all_time_low = 历史最低点

# Charts

charts_day = 1天
charts_hour = 1时
charts_week = 1周
charts_month = 1月
charts_year = 1年
charts_all = 全部
charts_entry = 入场价
charts_price_open = 开盘
charts_price_close = 收盘
charts_price_high = 最高
charts_price_low = 最低
charts_price_change = 涨跌幅

# Date

date_today = 今天
date_yesterday = 昨天

# Wallet Connect

wallet_connect_title = WalletConnect
wallet_connect_connection_title = 连接
wallet_connect_disconnect = 断开
wallet_connect_connect_title = 连接
wallet_connect_app = 应用
wallet_connect_brand_name = WalletConnect
wallet_connect_no_active_connections = 没有活跃的连接
wallet_connect_state_empty_description = 扫描二维码或粘贴连接链接，以连接 DApp
wallet_connect_permissions_title = 权限
wallet_connect_permissions_view_balance = 查看余额和交易记录
wallet_connect_permissions_approval_requests = 发送审批请求
wallet_connect_request_expired = 请求已过期

# Swap

swap_you_pay = 支付
swap_you_receive = 预计收到
swap_price_impact = 价格影响
swap_slippage = 滑点
swap_slippage_warning = 滑点过高。兑换可能以比预期更差的价格成交。
swap_slippage_auto = 自动
swap_slippage_auto_description = 自动设置最佳滑点以确保兑换成功。
swap_price_impact_warning_description = 本次交易将损失 %@ 的 %@。确定要继续吗？
swap_price_impact_warning_title = 价格影响
swap_estimated_time_title = 预计时间
swap_min_receive = 最少收到
swap_use_minimum_amount = 使用最低金额

# Stake

stake_active = 已激活
stake_pending = 待确认
stake_rewards = 奖励
stake_validator = 验证者
stake_validators = 验证者
stake_apr = 年化 %@
stake_available_in = 距可用
stake_inactive = 未激活
stake_lock_time = 锁定时间
stake_activating = 激活中
stake_deactivating = 解除质押中
stake_awaiting_withdrawal = 等待赎回
stake_active_in = 距激活
stake_minimum_amount = 最低金额
stake_state_empty_title = 质押将显示在这里
stake_state_empty_description = 首次质押 %@
stake_state_empty_validators_title = 未找到验证者
stake_resource = 资源
stake_resource_energy = 能量
stake_resource_bandwidth = 带宽
stake_delegations = 委托

# Assets

assets_no_assets_found = 未找到资产
assets_add_custom_token = 添加自定义代币
assets_select_asset = 选择资产
assets_popular = 热门资产
assets_tags_new = 新的
assets_state_empty_search_description = 请尝试手动添加
assets_title = 资产

# Sign Message

sign_message_message = 消息
sign_message_view_full_message = 查看完整消息

# Secret Phrase

secret_phrase_do_not_share_title = 切勿泄露助记词！
secret_phrase_do_not_share_description = 任何人只要拥有助记词，就能完全控制这个钱包！
secret_phrase_save_phrase_safely = 请将助记词保存在安全位置，\n且只有您可以掌控。
secret_phrase_confirm_quick_test_title = 完成这个快速测试，确认是否已正确保存所有内容。
secret_phrase_screenshot_detected_title = 检测到屏幕截图
secret_phrase_screenshot_detected_description = 屏幕截图可能会被其他应用访问；以此方式保存助记词会带来泄露风险。
secret_phrase_content_hidden_description = 屏幕录制过程中隐藏的内容

# Social

social_x = X（前 Twitter）
social_discord = Discord
social_reddit = 红迪
social_telegram = 电报
social_github = GitHub
social_youtube = 油管
social_links = 链接
social_facebook = 脸书
social_coingecko = CoinGecko
social_website = 网站
social_opensea = OpenSea
social_magiceden = MagicEden
social_instagram = Instagram
social_tiktok = 海外抖音
social_coinmarketcap = CoinMarketCap

# Nodes

nodes_import_node_title = 添加节点
nodes_import_node_chain_id = 链 ID
nodes_import_node_in_sync = 已同步
nodes_import_node_latest_block = 最新区块
nodes_import_node_latency = 延迟
nodes_gem_wallet_node = Gem 钱包节点
nodes_stream = 数据流
nodes_import_node_warning_message = 自定义节点可能存在恶意，可能会泄露交易数据或提供虚假信息。

# Library

library_select_from_photo_library = 从照片中选取

# Fee Rates

fee_rates_info = 交易速度取决于向矿工支付的网络费用
fee_rates_fast = 极速
fee_rates_normal = 普通

# Fee Rate

fee_rate_satvB = 聪/vB
fee_rate_satB = %@ 聪/B
fee_rate_gwei = gwei
fee_rate_custom = 自定义

# Banner

banner_account_activation_title = 账户激活费用
banner_account_activation_description = %@ 网络需要一次性费用 %@。
banner_stake_title = 开始质押 %@
banner_stake_description = 质押 %@ 赚取奖励，躺着也赚
banner_activate_asset_description = 要使用 %@ 资产，必须先在 %@ 网络上启用它，并满足该网络的特定要求。
banner_asset_status_title = 可疑资产
banner_asset_status_description = 代币可能不安全或具有误导性。请仅在完全信任的情况下继续操作。
banner_onboarding_title = 钱包已准备就绪
banner_onboarding_description = 购买或接收加密货币即可开始
banner_perpetuals_title = 在 Hyperliquid 上交易永续合约
banner_perpetuals_description = 在 Hyperliquid 上存入资产、交易永续合约并赚取收益

# Filter

filter_clear = 清除
filter_title = 筛选
filter_types = 类型
filter_has_balance = 有余额

# Price Alerts

price_alerts_enabled_for = 已为%@启用价格提醒
price_alerts_disabled_for = 已禁用%@的价格提醒
price_alerts_get_notified_explain_message = 当关注的加密资产出现大幅波动时，立即收到通知
price_alerts_direction_over = 高于
price_alerts_direction_under = 低于
price_alerts_direction_increases_by = 上涨
price_alerts_direction_decreases_by = 下跌
price_alerts_set_alert_price_over = 当价格高于
price_alerts_set_alert_price_under = 当价格低于
price_alerts_set_alert_price_increases_by = 当价格上涨
price_alerts_set_alert_price_decreases_by = 当价格下跌
price_alerts_set_alert_set_target_price = 设定目标价格
price_alerts_set_alert_current_price = 当前价格
price_alerts_added_price_over = 已添加价格高于 %@ 时的提醒
price_alerts_added_price_under = 已添加价格低于 %@ 时的提醒
price_alerts_added_increases_by = 已添加价格上涨 %@ 时的提醒
price_alerts_added_decreases_by = 已添加价格下跌 %@ 时的提醒
price_alerts_state_empty_title = 价格提醒将显示在这里
price_alerts_state_empty_description = 请添加币种，即可开始追踪资产
price_alerts_auto_footer = 当价格发生重大变动时，警报就会触发。
price_alerts_set_alert_title = 设置价格提醒

# Lock

lock_immediately = 立刻
lock_one_minute = 1 分钟
lock_five_minutes = 5 分钟
lock_fifteen_minutes = 15 分钟
lock_one_hour = 1 小时
lock_six_hours = 6 小时
lock_require_authentication = 需要身份验证
lock_unlock = 解锁
lock_privacy_lock = 隐私锁
lock_footer = 保护此设备上的应用访问安全

# Verify

verify_phrase_title = 确认

# Warnings

warnings_externally_controlled_account = 此 %@ 账户可能由他人控制。除非确定该账户由您掌控，切勿向其转入资金，以免永久损失资产。

# Sell

sell_title = 出售%@

# Info

info_network_fee_title = 网络费用
info_watch_wallet_title = 观察钱包
info_watch_wallet_description = 这是一个您无法操作的钱包，但可以查看它的交易和资产变动。
info_payment_verification_title = 付款身份验证
info_payment_verification_description = 商家在付款前需要您提供更多信息。
info_network_fee_description = 在 %@ 网络上的每笔交易都需要以 %@ 支付给矿工的手续费，以处理交易，Gem Wallet 不收取此费用。网络手续费根据网络使用情况而变化。
info_transaction_error_description = 由于资金不足、输入无效或网络拒绝等错误，交易无法完成。请检查详情并重试。
info_transaction_pending_description = 交易已提交，正在等待网络确认。处理时间可能有所不同。请稍后查看更新。
info_estimated_confirmation_description = 此估算基于交易手续费和 %@ 网络当前的负载情况。实际确认时间可能变化，无法保证。
info_transaction_success_description = 交易已完成并已在网络上确认。可查看详情以核实状态。
info_lock_time_description = 解锁等待期（也称解质押期或解冻期）是指发起解除质押后，质押资产暂时无法使用的等待时间。
info_price_impact_description = 价格影响表示交易对价格的冲击。价格影响越大，收到的代币越少，可能由于流动性低或交易金额大。
info_slippage_description = 滑点是指交易预期价格与实际执行价格之间的差异。
info_asset_status_unverified_description = 未验证代币是指未经可信第三方服务充分验证的代币。它们可能因空投、转账或手动导入而出现在钱包中。
info_asset_status_suspicious_description = 可疑或垃圾代币会被识别为潜在的诈骗或有害资产。它们可能因空投、转账或手动导入而出现在钱包中。
info_balance_required_title = 需要%@
info_insufficient_network_fee_balance_description = 此交易需要 %@ 支付 %@ 网络费用，但只有 %@ 可用。还需要 %@ 才能继续。
info_insufficient_balance_title = 余额不足
info_balance_required_description = 此交易需要%@，但只有%@可用。还需要%@才能继续。
info_account_minimum_balance_title = 最低余额
info_perpetual_funding_payments_title = 资金费用
info_perpetual_funding_payments_description = 资金费用是交易者之间的定期支付，用于使永续合约价格接近标的资产现货价格。正资金费用表示多头支付空头，负资金费用表示空头支付多头。
info_perpetual_liquidation_price_title = 强平价格
info_perpetual_liquidation_price_description = 强平价格是当市场价格达到该水平时，仓位将被自动平仓以防止进一步亏损的价格。当市场价触及此价位，仓位被强制平仓，保证金将被扣除。
info_perpetual_open_interest_title = 未平仓量
info_perpetual_open_interest_description = 未平仓量是所有尚未结算的永续合约总价值，可反映市场活跃度和流动性。
info_no_quote_description = 无法获取所选代币对的报价，可能是由于交易金额过小、流动性不足或技术限制。
info_stake_reserved_title = 预留网络费
info_stake_reserved_description = 钱包中会留有少量资金，用于支付取消质押或领取奖励等操作的费用。
info_stake_apr_description = 年利率（APR）是质押加密货币的年度回报率。
info_stake_frozen_required_title = 冻结 TRX 以质押
info_stake_frozen_required_description = 在 TRON 上质押分两步。先冻结 TRX，然后质押以赚取奖励。
info_perpetual_auto_close_description = 以设定的价格自动平仓。止盈锁定收益，止损限制损失。
info_fully_diluted_valuation_title = 完全稀释估值
info_fully_diluted_valuation_description = 完全稀释估值是所有代币均进入流通时的理论市值，计算方式为当前价格乘以最大供应量。
info_circulating_supply_description = 目前市场上可供交易的加密货币数量。
info_total_supply_description = 代币总供应量，包括已锁定或预留的部分。
info_max_supply_title = 最大供应量
info_max_supply_description = 该代币可能存在的最大数量。
info_perpetual_funding_apr_title = 资金费率年化
info_perpetual_funding_apr_description = 多头向空头支付的年化资金费率（为负时由空头向多头支付）。资金费是用户之间的点对点支付，不另收手续费，用于使永续合约价格接近现货价格。
info_minimum_amount_title = 最低金额
info_minimum_amount_description = 在 %@ 网络上，此交易的最低金额为 %@。
info_swap_minimum_amount_description = %@ 最低需要%@，但金额为%@。请增加%@以继续。

# Input

input_enter_amount_to = 输入%@的数量

# Nft

nft_properties = 特性
nft_collections = NFT 系列
nft_collection = 系列
nft_save_to_photos = 保存到照片
nft_set_as_avatar = 设为头像
nft_state_empty_title = NFT 将显示在这里
nft_state_empty_description = 接收第一个 NFT
nft_report_reason_spam = 垃圾信息
nft_report_reason_malicious = 恶意
nft_report_reason_inappropriate = 不当内容
nft_report_reason_copyright = 版权
nft_report_reason_other = 其他
nft_report_report_button_title = 举报

# Permissions

permissions_access_denied = 拒绝访问
permissions_image_photo_access_denied_description = 此应用无权访问照片库。请在设备设置中启用访问权限。

# Markets

markets_title = 市场
markets_state_empty_title = 市场信息将显示在此处
markets_daily_volume = 24小时交易量

# Search

search_state_empty_description = 检查拼写或尝试新的搜索。

# Networks

networks_state_empty_search_title = 未找到任何网络

# Onboarding

onboarding_security_create_wallet_intro_title = 您将获得一组助记词，请务必妥善备份。
onboarding_security_create_wallet_do_not_share_title = 切勿泄露助记词
onboarding_security_create_wallet_do_not_share_subtitle = 任何人拿到助记词，都能控制钱包并转走资产。
onboarding_security_create_wallet_keep_safe_subtitle = 请抄写助记词，并保存在只有您能取用的安全位置。
onboarding_security_create_wallet_no_recovery_subtitle = 请保管好备份，以免设备丢失或损坏后无法恢复钱包。
onboarding_security_create_wallet_keep_safe_title = 妥善备份助记词
onboarding_security_create_wallet_no_recovery_title = 我们无法找回助记词
onboarding_accept_terms_title = 接受条款
onboarding_accept_terms_message = 请阅读并同意以下条款后继续。
onboarding_accept_terms_item1_message = 我明白钱包的安全备份由我自行负责，Gem 不承担任何责任。
onboarding_accept_terms_item2_message = 我明白 Gem 不是银行或交易所，严禁将其用于非法用途。
onboarding_accept_terms_item3_message = 我明白如果丢失钱包访问权限，Gem 不承担任何责任，也无法提供帮助。
onboarding_accept_terms_continue = 同意并继续

# Brand


# Perpetual

perpetual_positions = 仓位
perpetual_size = 持仓量
perpetual_margin = 保证金
perpetual_position = 仓位
perpetual_close_position = 平仓
perpetual_pnl = 盈亏
perpetual_long = 做多
perpetual_short = 做空
perpetual_entry_price = 入场价格
perpetual_modify = 调整
perpetual_reduce_position = 减仓
perpetual_increase_position = 增加仓位
perpetual_open_direction = 开仓 %@
perpetual_close_direction = 平仓 %@
perpetual_leverage = 杠杆
perpetual_auto_close = 自动平仓
perpetual_auto_close_take_profit = 止盈
perpetual_market_price = 市价
perpetual_take_profit = TP
perpetual_stop_loss = SL
perpetual_liquidation = Liq
perpetual_increase_direction = %@ 加仓
perpetual_reduce_direction = %@ 减仓
perpetual_auto_close_expected_profit = 预期利润
perpetual_auto_close_expected_loss = 预期损失
perpetual_modify_position = 修改仓位
perpetual_auto_close_stop_loss = 止损
perpetual_value = 价值
perpetual_unrealized_pnl = 未实现盈亏
perpetual_volume = 交易量
perpetual_all_time_pnl = 总损益
perpetual_margin_usage = 保证金使用率
perpetual_account_leverage = 账户杠杆
perpetual_margin_cross = 全仓
perpetual_margin_isolated = 逐仓

# Perpetuals

perpetuals_title = 永续合约
# Used in the wallet screen row that opens Perpetuals when the wallet holds no positions.
perpetuals_trade = 交易永续合约
perpetuals_markets = 市场
perpetuals_empty_state_no_markets_found = 未找到任何市场

# Rewards

rewards_title = 积分
rewards_share_text = 通过我的邀请链接加入 Gem Wallet，即可获得积分：%@
rewards_my_referral_code = 我的邀请码
rewards_invite_friends_title = 邀请好友
rewards_invite_friends_description = 每邀请一位好友加入，即可赚取 %@ 积分。
rewards_referral_code = 邀请码
rewards_invited_by = 邀请者
rewards_points = 积分
rewards_referrals = 邀请人数
rewards_earn_points_title = 赚取积分
rewards_get_rewards_title = 兑换积分
rewards_activate_referral_code_description = 有朋友的邀请码吗？
rewards_activate_referral_code_title = 兑换邀请码
rewards_create_referral_code_title = 创建用户名
rewards_create_referral_code_info = 为当前钱包设置一个用户名。
rewards_username = 用户名
rewards_ways_spend_title = 积分兑换
rewards_ways_spend_asset_title = 兑换 %@
rewards_insufficient_points = 积分不足
rewards_confirm_redeem = 兑换 %@ 需要 %@！
rewards_pending_title = 积分待发放
rewards_pending_description = 预计 %@ 可领取
rewards_pending_description_ready = 积分已可领取！
rewards_nickname = 昵称
rewards_unverified_title = 积分尚未激活。
rewards_unverified_description = 继续使用应用，积分功能会自动激活。

# Recent Activity

recent_activity_title = 近期
recent_activity_state_empty_title = 最近的记录将显示在这里
recent_activity_state_empty_description = 最近使用过的资产将显示在此处

# Notifications

notifications_inapp_state_empty_title = 暂无通知
notifications_inapp_state_empty_description = 可以在这里查看通知更新。

# Earn

earn_state_empty_title = 持仓将显示在此处。
earn_state_empty_description = 首次存入 %@

# Contacts

contacts_title = 联系人
contacts_contact = 联系人
contacts_addresses = 地址
contacts_state_empty_title = 暂无联系人
contacts_state_empty_description = 保存常用地址
contacts_create_new_contact = 新建联系人
contacts_add_to_existing_contact = 添加到联系人

# Simulation

simulation_header_unlimited_asset = 无限 %@
simulation_warning_unlimited_token_approval_description = 此授权会允许被授权方持续使用该代币，直到您撤销授权或授权到期。
simulation_warning_externally_owned_spender_description = 此授权允许普通钱包地址 (EOA) 使用你的资产。仅在你信任该地址时确认。
simulation_warning_nft_collection_approval_title = 请求访问整个 NFT 系列
simulation_warning_unlimited_token_approval_title = 无限额度代币授权

# Application

application_name = Gem

# Camera Permission


# Notifications Permission


# Confirm


# Rootcheck

rootcheck_security_alert = 安全警告
rootcheck_jailbreak_body = 你的设备似乎已越狱。这可能使你的钱包和资金面临风险。
rootcheck_body = 检测到设备已获取 root 权限，可能显著增加安全风险。在已 root 的设备上使用此应用可能导致资产遭到未经授权的访问并造成损失。为保障资金安全，强烈建议使用非 root 设备。
rootcheck_ignore = 忽略
rootcheck_exit = 退出
