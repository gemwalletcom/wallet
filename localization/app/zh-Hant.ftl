...
common_show = 顯示%@
common_copied = 已複製：%@
common_copied_to_clipboard = 已複製到剪貼簿
...
common_lists = 清單
common_unpin = 取消置頂
common_pinned_asset = 已置頂：%@
common_unpinned_asset = 已取消置頂：%@
common_skip = 跳過
common_warning = 警告
...
wallet_create_new_wallet = 建立新錢包
wallet_import_existing_wallet = 匯入現有錢包
wallet_send = 傳送
wallet_receive = 接收
wallet_buy = 購買
wallet_swap = 兌換
wallet_import_title = 匯入錢包
wallet_new_title = 新建錢包
wallet_name = 名稱
wallet_import_action = 匯入
wallet_multicoin = 多幣種
wallet_import_address_field = 地址或域名
wallet_manage_token_list = 管理代幣
wallet_scan_qr_code = 掃描 QR 碼
wallet_scan = 掃描
wallet_scan_hint = 傳送、付款或連線至 DApp
wallet_scan_hint_address = 錢包地址
wallet_default_name = 錢包 #%d
wallet_default_name_chain = %@ 錢包 #%d
wallet_copy_address = 複製地址
wallet_watch_tooltip_title = 僅供觀察的錢包。這些資金不受您控制。
wallet_watch_empty_state_title = 正在觀察此錢包。
wallet_stake = 質押
wallet_receive_no_memo_required = 無需備註
wallet_receive_no_destination_tag_required = 不需要目的地標籤
wallet_title = 錢包
wallet_add_token_title = 新增代幣
wallet_sell = 出售
wallet_receive_collection = 接收 NFT 藏品
wallet_more = 更多
wallet_import_contract_address_field = 合約或代幣ID
wallet_import_address_warning = 可以查看此地址的餘額和交易，但**不能發送或出售資金**。
wallet_deposit = 轉入
wallet_withdraw = 提領
...
wallet_from_your_wallet_assets = 使用錢包中的資產
...
transfer_reserved_fees = 已保留 %@ 的餘額，以支付後續網路手續費。
...
settings_website = 前往網站
settings_security = 安全設定
...
support_state_empty_title = 需要什麼協助？
support_state_empty_description = 有任何問題都可以問我們。我們隨時提供協助。
...
errors_import_invalid_secret_phrase_word = 助記詞中包含無效單字：%@
errors_import_invalid_private_key = 無效的私鑰
errors_keystore_access = 無法在此裝置上存取該錢包的金鑰。如持有該錢包的助記詞，請移除該錢包並重新匯入以恢復存取。
...
errors_decoding_qr = 無法辨識 QR 碼，請改用其他 QR 碼再試一次。
...
errors_no_data_available = 無可用資料
errors_dust_threshold_short = 此網路將這筆金額視為粉塵金額，手續費高於金額本身。
...
activity_title = 交易紀錄
activity_state_empty_title = 交易將顯示在此處
activity_state_empty_description = 進行第一筆交易
activity_state_empty_search_description = 清除篩選條件以重新整理交易紀錄
activity_state_empty_search_title = 未找到任何交易

# Update App

update_app_title = 有新版本可用！
update_app_description = App %@ 版本現已推出。立即更新，享受最新功能與改進。
...
transaction_recipient = 收款人
transaction_sender = 傳送者
transaction_date = 日期
transaction_view_on = 在 %@ 上查看
transaction_status = 狀態
transaction_estimated_confirmation = 預計確認時間
...
asset_circulating_supply = 流通供應量
...
asset_verification_warning_title = 了解正在新增的內容
asset_verification_warning_message = 任何人都可以建立代幣，請謹防仿冒或惡意代幣。
asset_buy_asset = 購買%@
asset_get_asset = 取得 %@
...
charts_all = 全部
charts_entry = 進場價
charts_price_open = 開盤價
charts_price_close = 收盤價
charts_price_high = 最高價
charts_price_low = 最低價
charts_price_change = 漲跌幅
...
wallet_connect_connection_title = 連線
wallet_connect_disconnect = 中斷連線
wallet_connect_connect_title = 連線
wallet_connect_app = 應用程式
wallet_connect_brand_name = WalletConnect
wallet_connect_no_active_connections = 目前沒有有效連線
wallet_connect_state_empty_description = 掃描 QR 碼或貼上連線連結，以連線至 DApp
...
swap_slippage_auto_description = 自動設定最佳滑點以確保兌換成功。
...
stake_apr = 年化 %@
stake_available_in = 距離可用
stake_inactive = 未啟用
stake_lock_time = 鎖定時間
stake_activating = 啟用中
stake_deactivating = 解除質押中
stake_awaiting_withdrawal = 等待贖回
stake_active_in = 距離啟用
stake_minimum_amount = 最低金額
stake_state_empty_title = 質押將顯示在這裡
stake_state_empty_description = 首次質押 %@
stake_resource = 資源
...
secret_phrase_save_phrase_safely = 請將助記詞儲存在安全位置，\n且只有您可以掌控。
secret_phrase_confirm_quick_test_title = 完成這個快速測試，以確認已正確儲存所有內容
secret_phrase_screenshot_detected_title = 偵測到螢幕截圖
secret_phrase_screenshot_detected_description = 螢幕截圖可能會被其他應用程式存取；以此方式儲存助記詞會帶來外洩風險。
secret_phrase_content_hidden_description = 螢幕錄製過程中隱藏的內容

# Private Key

private_key_reveal_intro = 您即將顯示 %@ 帳戶的私鑰。
private_key_do_not_share_title = 切勿洩漏私鑰！
private_key_do_not_share_description = 任何取得此私鑰的人都可以完全控制您的 %@ 帳戶。
private_key_one_chain_title = 僅解鎖一條鏈
private_key_one_chain_description = 此私鑰僅控制 %@ 帳戶。
private_key_shared_chains_title = 解鎖多條鏈
private_key_shared_chains_description = 此私鑰控制您在 %@ 上的帳戶。
private_key_scope_evm = 所有 EVM 鏈
private_key_scope_cosmos = 所有 Cosmos 鏈
...
nodes_import_node_warning_message = 自訂節點可能是惡意節點，並可能洩露交易資料或提供不實資訊。

# Library

library_select_from_photo_library = 從照片圖庫中選取
...
banner_onboarding_title = 錢包已準備就緒
banner_onboarding_description = 購買或接收加密貨幣即可開始
banner_perpetuals_title = 在 Hyperliquid 上交易永續合約
banner_perpetuals_description = 在 Hyperliquid 上存入資金、交易永續合約並賺取收益
...
price_alerts_get_notified_explain_message = 喜愛的加密資產價格大幅變動時收到通知。
...
info_estimated_confirmation_description = 此估算是根據交易手續費和 %@ 網路目前的負載情況得出。實際確認時間可能變動，無法保證。
info_transaction_success_description = 交易已完成並在網路上確認。可以查看詳細資訊來驗證其狀態。
info_lock_time_description = 解鎖等待期（也稱解質押期或解凍期）是指發起解除質押後，質押資產暫時無法使用的等待時間。
...
info_asset_status_unverified_description = 未驗證代幣是指未經可信任第三方服務充分驗證的代幣。它們可能因空投、轉帳或手動匯入而出現在錢包中。
info_asset_status_suspicious_description = 可疑或垃圾代幣會被識別為潛在的詐騙或有害資產。它們可能因空投、轉帳或手動匯入而出現在錢包中。
info_balance_required_title = 需要%@
info_insufficient_network_fee_balance_description = 此交易需支付 %@ 作為 %@ 網路手續費，但目前只有 %@ 可用，還差 %@ 才能繼續。
info_insufficient_balance_title = 餘額不足
info_balance_required_description = 此交易需要%@，但只有%@可用。還需要%@才能繼續。
...
info_perpetual_liquidation_price_description = 強制平倉價格是市場價格達到某一價位時，倉位將被自動平倉以防止進一步損失的價格。當市場價格觸及此價位，倉位將被強制平倉，並損失保證金。
info_perpetual_open_interest_title = 未平倉量
info_perpetual_open_interest_description = 未平倉量是所有尚未結算的永續合約總價值，可反映市場活躍程度和流動性。
info_no_quote_description = 無法提供所選代幣對的報價，可能是因為金額過低、流動性不足或技術限制。
info_stake_reserved_title = 預留網路費用
info_stake_reserved_description = 錢包中會留有少量資金，用於支付取消質押或領取獎勵等操作的費用。
...
info_fully_diluted_valuation_description = 假設所有代幣均已流通時的理論市值，計算方式為價格乘以最大供應量。
info_circulating_supply_description = 目前市場上可供交易的加密貨幣數量。
info_total_supply_description = 目前存在的代幣總數，包括鎖定或預留的代幣。
info_max_supply_title = 最大供應量
info_max_supply_description = 代幣可能存在的最大總量。
info_perpetual_funding_apr_title = 年化資金費率
...
input_enter_amount_to = 請輸入要%@的數量

# Nft

nft_properties = 屬性
nft_collections = NFT 系列
nft_collection = 系列
...
permissions_access_denied = 存取遭拒
permissions_image_photo_access_denied_description = 此應用程式無權存取照片圖庫。請在裝置設定中啟用存取權限。
...
onboarding_accept_terms_continue = 同意並繼續
...
perpetual_open_direction = 開倉 %@
perpetual_close_direction = 平倉 %@
perpetual_leverage = 槓桿
perpetual_auto_close = 自動平倉
...
perpetual_increase_direction = %@ 加倉
perpetual_reduce_direction = %@ 減倉
...
rewards_title = 積分
rewards_share_text = 透過我的邀請連結加入 Gem Wallet，即可獲得積分：%@
rewards_my_referral_code = 我的邀請碼
rewards_invite_friends_title = 邀請好友
rewards_invite_friends_description = 每邀請一位好友加入，即可獲得 %@ 積分。
rewards_referral_code = 邀請碼
rewards_invited_by = 邀請者
rewards_points = 積分
rewards_referrals = 邀請人數
rewards_earn_points_title = 賺取積分
rewards_get_rewards_title = 兌換積分
...
rewards_insufficient_points = 積分不足
rewards_confirm_redeem = 兌換 %@ 需要 %@！
rewards_pending_title = 積分待發放
rewards_pending_description = 預計 %@ 可領取
rewards_pending_description_ready = 積分已可領取！
rewards_nickname = 暱稱
...
recent_activity_state_empty_title = 最近的記錄將顯示在這裡
recent_activity_state_empty_description = 最近使用過的資產將顯示在這裡

# Notifications

notifications_inapp_state_empty_title = 暫無通知
notifications_inapp_state_empty_description = 可以在這裡查看通知更新。

# Earn

earn_state_empty_title = 持倉將顯示在此。
earn_state_empty_description = 首次存入 %@

# Contacts

contacts_title = 聯絡人
contacts_contact = 聯絡人
contacts_addresses = 地址
contacts_state_empty_title = 尚無聯絡人
contacts_state_empty_description = 儲存常用地址
...
simulation_warning_unlimited_token_approval_description = 此授權允許被授權方持續使用此代幣，直到您撤銷授權或授權到期。
simulation_warning_externally_owned_spender_description = 此授權允許一般錢包位址（EOA）使用您的資產。僅在您信任此位址時確認。
simulation_warning_nft_collection_approval_title = 要求存取整個 NFT 系列
simulation_warning_unlimited_token_approval_title = 無限額代幣授權
